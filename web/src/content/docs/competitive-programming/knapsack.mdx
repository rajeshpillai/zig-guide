...
So check the bounds before reaching for this table.

If `W` is up to about 10^5 or 10^6, the table fits.

If `W` is up to 10^9 and the values are small, the usual move is to swap the roles: index the table by total value and store the least weight that reaches it.
