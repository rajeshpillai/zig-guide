...
seoDescription: Binary search in Zig, with the search printing every step. The half-open window and its invariant, the off-by-one that stops the loop ending, lower bound against upper bound, and std.sort.lowerBound with the comparator argument order that is easy to get wrong.
