...
The two `while` loops inside the `for` loop are still linear, for the same reason as in [the monotonic stack](/learn/competitive-programming/monotonic-stack/).
...
[Sliding Window](/learn/competitive-programming/sliding-window/) uses the same kind of amortised argument.
