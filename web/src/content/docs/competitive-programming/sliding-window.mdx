...
description: One addition and one subtraction instead of a fresh sum, the window that grows and shrinks, and the count that shows the inner loop keeps the total linear.
...
The `while` loop inside the `for` loop is still O(n), for the reason explained in [The Monotonic Stack](/learn/competitive-programming/monotonic-stack/).
