...
same Zig, with the same syntax and the same standard library functions, and it
is debugged by reading it rather than by expanding a macro. That is the whole
claim, and it is what makes the feature worth its weight: C++ templates, C
macros and Rust's declarative macros are each a separate language with separate
rules, and none of them can be stepped through.
...
Every call with a different `seed` gets its own compiled copy with the constant
folded in. Which is also the cost: `comptime` parameters multiply the amount of
code generated, so they belong on things that take a handful of distinct values,
not on things that take a thousand.
...
Calling the same function with the same arguments gives the same type back, not
a new one each time, which is what makes `List(u8)` in two different files refer
to one thing.

## What can and cannot run at compile time

Anything that is pure computation can: arithmetic, control flow, building
arrays, walking a type's fields, formatting a string. Anything that touches the
outside world cannot. There is no reading a file, no clock, no allocation from a
runtime allocator, and no calling into C.
...
before an expression demands it be evaluated now and fails the build if it
cannot, which turns a silent fallback to runtime into an error.

The mirror image is `@compileError`, which fails the build with your message
when a branch is reached during compilation. Combined with `@typeInfo`, that is
how a generic function rejects a type it cannot handle with a sentence instead
of an error inside its body.
...
them costing a test at runtime, and a debug-only code path can be written
...
type you instantiate is work the compiler does on every build, and it is
possible to write a program that is fast at runtime and unbearable to build.
When a comptime computation starts needing a large branch quota, that is the
signal.
