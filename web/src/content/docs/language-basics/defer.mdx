...
inside a loop and deferring its close closes it on each pass, so the loop cannot
...
A deferred statement runs on the way out of a scope. Letting it pick a different
way out would make the exit path of every function with cleanup unreadable.
...
`defer` does the work of a destructor, with the pairing written at the use site
instead of attached to a type. The trade is deliberate. Nothing runs invisibly
at a closing brace: everything that happens on the way out is a statement in the
function you are reading. The cost is that a `defer` can be forgotten, which is
why the [allocator chapter](/learn/standard-library/allocators/) leans on a
leak-checking allocator to catch what review misses.
