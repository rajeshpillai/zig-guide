...
seoDescription: "Zig has no truthy values: if takes a bool, and nothing else converts to one. if is also an expression, which is what replaces the ternary operator."
