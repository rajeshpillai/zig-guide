...
is that the common case carries no opportunity for an off-by-one. You do not
write the bound, so you cannot write it wrong.
...
rather than a copy of the array, and `|*n|` is what makes each capture a pointer
into it. Leave off either and the loop compiles and does nothing useful.
...
forward loop is the one that is safe by construction, and going backwards means
opting back into managing the index yourself.
...
for the ran-out-of-elements case, which makes linear search a single expression.
See [loops as expressions](/learn/language-basics/loops-as-expressions/) and
[labelled loops](/learn/language-basics/labelled-loops/).

`inline for` unrolls the loop at compile time and makes the capture
comptime-known, which is what lets a loop iterate over the fields of a struct or
the tags of an enum. That is a different tool with a different purpose, covered
in [inline loops](/learn/language-basics/inline-loops/).
