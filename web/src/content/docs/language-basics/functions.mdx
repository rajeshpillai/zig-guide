...
long function, `x` on line 60 is the `x` that was passed in, and you do not have
to read the 59 lines above to be sure.
...
parameters too, which is why `_ = name;` shows up in half-written code: the
compiler is refusing to let a variable you stopped using survive a refactor
unnoticed.

A function returning `void` is the exception. There is nothing to use, so a bare
call is fine.

## Generic functions are ordinary functions

A parameter typed `anytype` is filled in from the call site, and the function is
compiled once for each set of types it is called with:
...
`maxOf(@as(u8, 3), 5)` and `maxOf(@as(f32, 1.5), 0.5)` both work, and each gets
its own machine code. There is no separate generics syntax, no type parameter
list and no constraint language, because a type is a value at compile time and a
function that takes types is just a function. That idea runs through the whole
standard library; [comptime](/learn/language-basics/comptime/) is where it is
taken apart.

The trade is that the errors arrive at instantiation rather than at definition.
A function that calls `a.len` on its `anytype` parameter compiles fine until
someone passes an integer, and then the message points inside your function
rather than at their call.
...
This is what a callback field in a struct holds, and it is how the interfaces in
the standard library are built: a pointer to some state, plus a table of
...
The failure mode is worth knowing, because it is not one you can catch. A stack
overflow is a crash, not a Zig error value, and how loudly it fails depends on
the platform. Anywhere the depth is decided by input, the recursion depth is an
