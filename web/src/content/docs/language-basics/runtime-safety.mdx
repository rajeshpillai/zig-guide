...
Snippets on this site build as `ReleaseSmall`, where the checks are off, so the
...
against the 79 KB a snippet here typically weighs, which is what the checks and
the machinery to describe them cost, and it downloads only when you press Run.
