...
literal type, which coerces to any enum that has a matching tag. That is why the
same `.north` works for two unrelated enums in two different calls.
...
cannot fail, because every value is representable, and a `switch` handles the
unnamed case with `_` rather than `else`, which keeps the exhaustiveness check
alive for the named ones. See [switch](/learn/language-basics/switch/).
...
They can hold constants and nested types too, so an enum is a fine place to put
the parsing and formatting that belong to it. A `fromString` returning `?Suit`
next to a `toString` returning `[]const u8` keeps the whole mapping in one
place, and `std.meta.stringToEnum(Suit, text)` writes the parsing half for you
from the tag names.
...
or parses enums without a hand-written table. Walk `field_names` with
`inline for` and you have a parser, a formatter or a table of every case, built
from the declaration and unable to fall out of sync with it. See
...
attached. When the cases are independent flags that can combine, an enum is the
wrong shape and a `packed struct` of `bool` fields is the right one, since it
gives you named bits backed by a single integer.
