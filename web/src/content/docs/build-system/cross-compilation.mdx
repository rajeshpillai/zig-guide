...
cost, which is why "we only build on Linux" is a common answer and why CI
...
it needs on demand, so the toolchain and the sysroot are already inside the
single binary you downloaded. Building a Windows executable from a Mac is the
same command as building for yourself, with six more characters.

## What still needs care

Cross-compiling gets you a binary. It does not get you a tested one, and it
does not remove the platform differences the code has to handle: path
separators, line endings, the fact that `usize` is 4 bytes on wasm and 8 on
your laptop. The build succeeding proves the code compiles for that target,
which is genuinely most of the value and is not the same as working.
...
substitution, without a line of its source changing. `zig c++` does the same
for C++.
