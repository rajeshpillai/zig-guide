...
Which means somebody has to be responsible for handing it back, and the whole
of memory management is that one question: **who owns this, and when do they
give it up?** Every scheme you have heard of is an answer to it. A garbage
collector answers "nobody, the runtime works it out later". Rust answers "the
compiler tracks it". C answers "you do, good luck". Zig answers "you do, and
the type signature will say so".
...
reach for. So a function that needs memory cannot hide it: it is in the
signature, visible at every call site, and a function without that parameter
cannot be quietly allocating behind your back.
...
systems programming expect. Plenty of jobs that look like they need the
heap just need the caller to supply the space.
...
the release for when the block ends, however it ends. The two lines are next to
each other, so you can see the pair at a glance instead of scrolling to a
cleanup section at the bottom and hoping every path reaches it.
...
That last part is the reason to learn ownership in Zig rather than in C. A leak
is not something you discover in production three months later when a server
falls over. It is a number the program prints before it exits, and in tests it
is a red build: `std.testing.allocator` fails any test that leaks.
