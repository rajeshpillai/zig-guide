...
So how does the machine know that one byte is the letter `A` and another is the
number 65?

It does not. They are the same byte. What differs is the **type**, and a type
is an agreement about how to read a run of bytes: how many of them belong
...
That is the whole idea, and it carries further than it sounds. Almost everything
that feels mysterious later, why a file and an image are the same thing to the
operating system, why a network sends numbers and receives text, comes back to
this one sentence.
...
came first. This machine stores the smallest place value at the lowest address,
which is called **little-endian**. It is a choice the hardware made, not a rule
of arithmetic. Most machines you will meet do it this way, and it stops being
surprising once you have seen it once. It matters in exactly two places, both
much later: reading a file someone else wrote, and sending bytes over a network.
...
space they need. (It is not always exactly the sum, for reasons the
[memory layout](/learn/how-to/memory-layout/) recipe covers, but for a start it
is a good model.)

The **Edit** button opens the snippet so you can change it. Running edited code
needs a Zig compiler this deployment does not carry yet, so it will offer to
copy the source out to an external playground instead. Every unedited snippet
runs here.
...
repeatedly: the same operation is available, and the mistake next to it is not.
