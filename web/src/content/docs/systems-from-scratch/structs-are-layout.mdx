...
four-byte number that starts at an address divisible by four can be read in one
go. One that straddles the boundary takes two reads and some assembly, and on
some processors it is not allowed at all. So the rule is that a value of size N
starts at an address divisible by N.
...
struct` is how you ask for C's rules, and you ask for them when the layout has
to match something outside your program, a C library or a file format or a
network packet. Inside your own program, let the compiler do it.
...
aligned to its largest field, which is what makes arrays of it work. Adding one
8-byte field to a 8-byte struct cost 8 bytes, and that is the ordinary case
...
and `packed struct`; a plain struct is the compiler's business. That is a
better default (most structs never leave your program, and packing them is free
performance) and a worse one exactly once: the moment you assume otherwise at
an FFI boundary. Reach for `extern struct` there and the two languages agree.
...
the C ABI in detail, are in the
[memory layout](/learn/how-to/memory-layout/) recipe.

Next: [Errors Are Values](/learn/systems-from-scratch/errors-are-values/), which
is about what a function returns when it cannot do what you asked.
