...
`std/mem.zig`. Code written against them compiles and behaves identically, which
is exactly why this is worth a section: nothing fails, so a guide can teach the
old spelling for another year and never find out.
