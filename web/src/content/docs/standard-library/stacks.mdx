...
uses it: a parser with a bounded nesting depth has no error path for "ran out of
stack", only a check against the limit. That check is the thing to write, and it
is the difference between rejecting deeply nested input and crashing on it.
...
somewhere the cache knows nothing about, and waits. Textbooks present the two as
equivalent because they compare instruction counts, which stopped being the
...
where the extra allocation buys you something.
