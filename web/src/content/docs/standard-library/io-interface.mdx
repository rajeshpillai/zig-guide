...
a real thread pool; [Choosing an Io](/learn/standard-library/choosing-an-io/) covers
the others and when you would swap. It is the same dependency-injection pattern the allocator
uses, applied to blocking instead of to memory. That is why this chapter sits
next to [Allocators](/learn/standard-library/allocators/): learn the two together
and the rest of the library stops surprising you.
...
The [Concurrency](/learn/standard-library/concurrency/) chapter builds on this with
`Future` and `Group`, and [Cancellation](/learn/standard-library/cancellation/) with
stopping a task once it is running; [Readers and Writers](/learn/standard-library/readers-and-writers/),
[Filesystem](/learn/standard-library/filesystem/), and [Time](/learn/standard-library/time/)
are each one corner of the same interface.
...
with no `Io` in sight now routes through `std.Io.Dir` and
`std.Io.Clock`. The free-standing, globally-available I/O functions are gone;
the [migration page](/learn/getting-started/coming-from-older-zig/) lists the
specific renames.
