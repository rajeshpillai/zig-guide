---
title: ArrayList
seoTitle: "Zig ArrayList: The Unmanaged Growable Array"
seoDescription: "std.ArrayList no longer stores its allocator, which is the change most likely to break code you find online. Appending, growing, and handing memory back."
...
[queue](/learn/concurrency/queues/) is not. When lookup is by key rather
than by position, use a [hash map](/learn/standard-library/hash-maps/). When
the element is a large struct and you mostly touch one field at a time, use a
[MultiArrayList](/learn/standard-library/multi-array-list/). It stores each
