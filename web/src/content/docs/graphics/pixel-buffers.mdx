...
description: Row-major layout, top-left origin, packed pixels, and a clipping putPixel.
section: Graphics
order: 120
---
...
There is no graphics library in this section. Every pixel is set by code you can
read, and the only thing between the code and an image is a block of numbers and
a decision about how to interpret it.
...
<Playground name="09-graphics.framebuffer" />

## Why the buffer is global

`640 * 480 * 4` is 1.2 MB. The default thread stack is 8 MB on Linux, 1 MB on
Windows, and smaller still on some embedded targets, so a full-size image
declared as a local is a portability bug waiting for a different machine. Make
it global, or heap-allocate it.

`undefined` is honest here only because `drawGradient` writes every cell before
anything reads one. A scene that fills part of the buffer has to start from a
known colour, or it ships whatever was in memory.
...
but the last, because integer division truncates. This is the most common bug in
integer scaling code, and it is invisible until you look at the output.
...
from its start. Half of those results are negative. In `usize` a negative result
underflows to an enormous number, which is a panic in Debug mode and a wrong
answer everywhere else. Coordinates that can go negative want a signed type, and
the `@intCast` at the point of use is the price.
...
[Circles](../circles/) is a handful of lines even when it hangs off three sides
of the buffer at once.

`putPixel` is also the slow way to draw. A real renderer writes whole spans of a
row at a time, because a per-pixel call and a per-pixel bounds check dominate the
cost. It is the right shape for learning and the wrong shape for a hot loop.
...
so what you count on screen is what the code wrote. The chapters after this one
share that dump through a helper module:
...
The order is: get pixels into a buffer, get the buffer out of the process, then
draw things worth looking at.
