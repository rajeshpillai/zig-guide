...
whether you are still alive, and reports input, and missing your side of any of
that leaves the window blank, frozen, or killed.
...
Open a display, create a window, map it, then answer events. It is markedly less
ceremony than Wayland, and it still runs everywhere, including under XWayland on
a Wayland desktop.
...
[Importing C](../../working-with-c/cimport/) for the general shape.
...
`addSystemCommand` step with `addOutputFileArg2`, so the generated files can live
