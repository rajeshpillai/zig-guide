...
<Playground
  ...
  note="Built and run natively by CI. wasm32-wasi has a single thread, so every worker would run inline and the program would behave like a plain for loop."
/>
...
Atomic operations cannot keep that pair consistent, as [Atomics](/learn/concurrency/atomics/) explains at its end.
...
[Cancellation](/learn/concurrency/cancellation/) shows what goes wrong otherwise, including how it happened in this guide's own CI.
...
Work that arrives while the program is running needs [`Io.Queue`](/learn/concurrency/queues/) instead.

There, producers and consumers run at the same time, and the queue's capacity limits how far producers can get ahead.

Work with a deadline needs [Select](/learn/concurrency/select/) around the group, and then cancellation for whatever has not finished.
...
Next: [Choosing an Io](/learn/concurrency/choosing-an-io/) for the choice this program makes in one line, which the other chapters assume.
