...
seoDescription: "A complete concurrent Zig program: four workers over one batch, an atomic work index, a private result slot per record, and a mutex over the only state the workers share. Includes the measurement showing all twelve records ran on one thread, and why that is the correct outcome."
