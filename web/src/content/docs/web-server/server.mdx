...
where it actually hurts, because now the bytes are trickling in from a socket
...
on. Those two mechanisms are the only ways HTTP/1.1 has to end a body, and
chunked encoding is the third that exists because neither works for a response
...
and forgetting that is the off-by-one from the [strings
chapter](/learn/terminal/strings/) on a buffer holding attacker- controlled
bytes. And `write` can be partial on a socket just as on a pipe, so the C
version needs its own loop around it; `writeAll` is that loop.
