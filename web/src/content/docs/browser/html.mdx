...
`UnexpectedToken`. Refusing is how a parser tells you the input was not what it
claimed to be.
...
that every possible byte sequence produces, including sequences no author would
ever write on purpose.
...
The input contains four things that would be errors in XML. All four produced a
sensible tree.
...
The parser trims whitespace between tags and drops it if nothing is left. Where
would that be wrong?
...
The rules for which is which are genuinely fiddly, and they are the reason
...
parser repairs nesting, so C reaches for `malloc` per node and a linked list
per child list, and then owns freeing all of it when the document is thrown
away. The version here uses indices into one flat array, which means the whole
...
specification's tokenizer has around 80 states, and the tree construction stage
has a dozen "insertion modes" with different rules each. That is not
