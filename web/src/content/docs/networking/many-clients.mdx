...
stop. The traditional answers are a thread per connection, which costs a stack
each and stops scaling in the thousands, and a single-threaded event loop,
which scales and infects every function you write with callbacks.
...
That is the whole argument for writing protocol code against `std.Io.Reader`
and `std.Io.Writer` instead of against a socket: the tests get to be fast and
the browser gets to run the example.
...
[Threads](/learn/standard-library/threads/) is a page you cannot run here and this
one is.
...
not syntax. [Concurrency](/learn/standard-library/concurrency/) covers the shape in
general.
...
[Cancellation](/learn/standard-library/cancellation/) covers what a handler sees when
the group it belongs to is cancelled.
...
that connection's reply, which is the same conclusion the
[text protocol chapter](/learn/networking/text-protocols/) reached from the other
direction. One client sending garbage must not affect any other client.
