...
via `@import`. A test in a module nobody imports never runs — worth knowing
when a test you expected to fail stays quiet.
...
`return error.SkipZigTest` marks a test skipped rather than failed — the way to
handle a test that only applies to some targets.
