...
wrong within a year; writing against master means they are correct today and may
need a fix tomorrow. This guide takes the second trade and pays for it with a
nightly build that recompiles and reruns every snippet against a fresh compiler.

What that means for you as a reader: if something here does not compile on your
machine, check `zig version` first. A mismatch of a few weeks is usually
...
software, and [coming from an older Zig](/learn/getting-started/coming-from-older-zig/)
lists what differs so you can read this guide anyway.

## Keeping it current

Master moves daily. Updating weekly is a habit worth having, because the changes
arrive in small pieces that are each easy to absorb, whereas six months of them
...
There is also no package manager to install, no formatter to configure
(`zig fmt` is built in and has no options to argue about), and no separate
debugger integration to set up beyond pointing your existing debugger at the
binary. The download is the environment.
