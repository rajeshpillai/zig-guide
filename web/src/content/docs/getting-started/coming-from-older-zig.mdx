...
The whole `indexOf` family moved to `find`. `Allocator.print` is the allocating
formatter. `SafeAllocator` also changed shape: it takes its backing allocator as
an argument, and `deinit()` returns a leak count rather than `.ok` or `.leak`.
...
→ [Strings](/learn/standard-library/strings/) · [Formatting](/learn/standard-library/formatting/) · [Allocators](/learn/standard-library/allocators/)
