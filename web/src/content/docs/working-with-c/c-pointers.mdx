...
harmful anywhere else: it silently launders away the guarantees the rest of
...
C, is a sign that one leaked out of a boundary and has been travelling since.
The fix is at the boundary, not at the error.
