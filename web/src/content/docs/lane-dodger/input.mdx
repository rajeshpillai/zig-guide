---
title: Input
seoTitle: "raylib Input in Zig: Latching a Key Press to Exactly One Tick"
description: Presses latched once per frame and handed to exactly one tick, and why a held key must not cross the board.
seoDescription: "Reading raylib key presses in Zig and latching each one to exactly one simulation tick, so a held key cannot cross the board and a press is never handed to two ticks."
section: Lane Dodger
order: 70
---
