...
synthesiser. It would also have added a folder of binaries nobody can diff, a
licence question attached to each one, and a tuning loop that runs through an
audio editor. Generating them instead means a sound is changed by changing a
number.
...
the waveform happened to be at. A step like that is a broadband impulse, which
...
So the sounds are checked without an audio device, and CI has no speakers
anyway.
...
Brightness there is zero crossings per second, which is a crude pitch estimate
and good enough to tell a chime from a thud.

Measured, the five come out as intended: the coin at about 1130 Hz over 85 ms,
the crash running 900 ms with 81 percent of its energy below 500 Hz, and the
lane tick shortest and quietest. `zig build sounds` writes them all out as
...
Failing to start because a mixer would not open is not an acceptable outcome for
a game whose sound is decoration. The real binary is run with a deliberately
broken ALSA configuration as part of checking that.

None of this changed a rule of the game. It reads the same
