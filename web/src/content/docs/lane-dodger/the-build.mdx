...
description: Step by step, how to add raylib to a Zig project, why the dependency here is its C rather than its build script, and how the same source compiles to a browser canvas.
section: Lane Dodger
order: 20
---
...
raylib is a C library for windows, drawing and sound. This chapter adds it to a
Zig project.
...
That makes no difference. The whole build script has to type-check.

`raylib-zig`, the usual binding, is further behind: it pins raylib 5.5 and calls
three standard library functions that no longer exist.

Neither project is at fault here. They track releases and this guide tracks
master, and that gap is where the breakage lives. It is worth knowing about
before you take the dependency, not after.

## Depend on the C instead

raylib's C changes far more slowly than Zig's build API does. So depend on the
C.
...
Keep `src/` and the licence, delete the rest. raylib's `examples/` directory
alone is 72 MB and none of it gets compiled. `fetch-raylib.sh` in this project
does all of the above and records the commit it fetched.

Add `vendor/` to `.gitignore`. The script is the record, not the copy.
...
Seven C files. `rglfw.c` is raylib's bundled GLFW, the windowing layer. The rest
are raylib's modules, switched on and off by the `SUPPORT_MODULE_*` macros.
...
The macros are copied from raylib's own build script, and that is the running
cost of this approach: when raylib changes its configuration, this has to
follow. It is about forty lines in one function, and a mismatch fails at compile
time rather than quietly producing a different library.

## The same source, on a canvas

Only this one function differs between a desktop window and a browser.
...
`emcc` does the link, because the JavaScript glue, the GL context and the canvas
plumbing are things only Emscripten emits.
...
The browser build is a 200 KB `.wasm` plus about as much JavaScript. It needs
the Emscripten SDK:
