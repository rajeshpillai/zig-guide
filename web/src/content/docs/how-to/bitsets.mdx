...
description: A static bit set turns membership, intersection, and union into single instructions.
section: How-To
order: 99
---
...
scanning buckets. And because `bit_set.Static` picks its representation from
the size (a single integer up to pointer width, an array of words beyond
it), `std.bit_set.Static(365)` for days of a year works identically, just
