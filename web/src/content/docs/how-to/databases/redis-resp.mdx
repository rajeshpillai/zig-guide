...
in the same process, the way [the TCP recipe](/networking/tcp-echo/) does. CI runs
...
[the TCP recipe](/networking/tcp-echo/) connects. The encoding and decoding here
...
- **A persistent store:** for a database that keeps data on disk from Zig, see
  [SQLite from Zig](/how-to/databases/sqlite-basic/).
