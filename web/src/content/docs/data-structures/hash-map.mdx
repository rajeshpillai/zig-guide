...
description: Open addressing with linear probing, growth at 75%, and the tombstone, which looks like a hack but is what makes delete work.
...
[Hash maps](/learn/standard-library/hash-maps/) covers the API. The main structural difference is that `std` stores a separate array of one-byte
...
pattern [ArrayList](/learn/standard-library/arraylist/) uses on master: the
