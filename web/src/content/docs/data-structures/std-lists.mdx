...
That is what intrusive means. The container does not wrap your value; your value
contains the container's bookkeeping. The list then holds pointers to those
embedded nodes, and to get from a node back to the task it lives in:
...
the node really is inside a `Task`, which is why the conversion is written once,
next to the struct that owns it.
...
run queue, "insertion cannot fail" is not a convenience but a requirement:
...
lists at once. A wrapping list cannot express that without allocating a separate
node per membership.

**The cost is visible.** The node is a field in your struct, so the memory a
membership costs is written down where you can see it, in the type that pays it.
...
pointer widths because this page runs as wasm32 in your browser, where a pointer
is four bytes, and natively on your machine, where it is eight; the
relationships hold either way.

## What to use instead for ordinary work

For "a growable sequence of values", [ArrayList](/learn/standard-library/arraylist/).
It is contiguous, cache-friendly, and it is what `std` expects you to reach for.

The intrusive lists are there for the cases in the previous chapter's last
section: pointer stability, O(1) splicing, and insertion that must not allocate.
If you are writing an allocator, a scheduler, or an object pool, they are
exactly right. If you are collecting user records, they are not.
