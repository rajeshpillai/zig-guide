...
Two back ends, one language, and they are not equally complete. The tree-walker
runs functions and recursion; the compiler and this VM do not, because
compiling a call means designing a calling convention, and that is a chapter
rather than a paragraph. The compiler chapter says the same thing, and the
`fn` cases in both fall through an `else` that does nothing.

Being explicit about that is better than a language that appears to support
functions and silently drops them. If you want to carry this further, the
calling convention is the next thing to design: where arguments live, how a
frame is addressed relative to a base pointer, and who restores the instruction
pointer on return. The frames chapter already did all of that at run time, so
you know what the answers have to be.
