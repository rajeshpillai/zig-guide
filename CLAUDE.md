...
**One ordering.** [web/src/nav.ts](web/src/nav.ts) derives the track/section/group/chapter order, and everything that shows an order reads it: the sidebar, the home page, the routes and index listings in [[...slug].astro](web/src/pages/[...slug].astro), and the previous/next pager at the foot of every page.
