...
**One ordering.** [web/src/nav.ts](web/src/nav.ts) derives the section/group/chapter order from the collection, and everything that shows an order reads it: the sidebar, the routes and index listings in [[...slug].astro](web/src/pages/[...slug].astro), and the previous/next pager at the foot of every page. `readingOrder()` flattens it into the walk a reader following "Next" takes (a section's overview, its ungrouped chapters, then each group's overview and chapters), so moving a chapter moves it everywhere at once. The browser gate walks the whole chain: each "next" must be answered by the matching "previous", exactly two pages may be missing a link (the two ends), and following "next" from the first page must reach every page the sidebar lists. A pager that skipped a chapter or looped back would look perfectly reasonable on any single page.
