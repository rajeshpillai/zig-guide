...
- `//! fails` → built `.safe` instead of the site-wide `.small`, shipped to the browser with a Run button, and **required by CI to stop** with the text in a sibling `.expected-error` file. `zig build verify` fails if the snippet exits 0 or fails with different wording, so a panic message quoted in a chapter is checked nightly rather than remembered. Missing the sibling file is a configure-time panic, since a `//! fails` snippet with nothing to match would accept any failure at all, including the ones a bad rewrite introduces. Use it sparingly: `.safe` costs ~1.1 MB against ~46 KB, paid by anyone who presses Run on that snippet. Two snippets use it, both in the beginner track and both where the check firing *is* the lesson: `past-the-end` (out-of-bounds read) and `the-one-that-depends` (integer overflow). A third would want justifying.
