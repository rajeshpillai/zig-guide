...
- `//! fails` → built `.safe` instead of the site-wide `.small`, shipped to the browser with a Run button, and **required by CI to stop** with the text in a sibling `.expected-error` file. `zig build verify` fails if the snippet exits 0 or fails with different wording, so a panic message quoted in a chapter is checked nightly rather than remembered. Missing the sibling file is a configure-time panic, since a `//! fails` snippet with nothing to match would accept any failure at all, including the ones a bad rewrite introduces. Use it sparingly: `.safe` costs ~1.1 MB against the ~79 KB a snippet here typically weighs, paid by anyone who presses Run on that snippet. Three snippets use it, all where the check firing *is* the lesson rather than an illustration: `15-groundwork.past-the-end` (out-of-bounds read), `15-groundwork.the-one-that-depends` (integer overflow), and `02-language.runtime-safety`, the chapter about runtime safety, whose aside used to apologise for not being able to show a check fire. A fourth would want justifying, and the test is whether the chapter is *about* the check rather than merely mentioning it.
