...
`WaitGroup` and `Pool` are simply not on `std.Thread` any more. Everything
that waits took an `Io` and moved. `std.Thread.spawn` stayed, along with the
four other calls that are really about an OS thread, but structured
concurrency via `io.async` and `Io.Group` is the better default and works on
targets with no threads at all.
