...
A `test` block is part of the Zig language. You do not import a test
framework, register the test or add annotations to it.
...
via `@import`. A test in a module nobody imports never runs. So if a test you
expected to fail stays quiet, check that its file is imported.
...
You do not have to turn this on or run a separate tool. Every test that
allocates is a leak test:
...
`return error.SkipZigTest` marks a test as skipped instead of failed. Use it
for a test that only applies to some targets.

## Where to put them

Put tests next to the code they test, in the same file. The standard library
does this, and you should too, for two reasons.

First, the test can reach private declarations, because privacy is per file. A helper
...
Second, the test works as an example. Someone opens the file to see how a
function is called. A few lines below the definition, they find a call with
real arguments and a checked result. People do not read documentation in a
separate directory that way.

Tests are not compiled into your program. A `test` block only exists in a
build produced by `zig test` or a test step. So leaving them in place costs nothing, and
you do not need to move them at release time.
...
The runner prints that name on failure, and `--test-filter` matches it. So a
description of the behaviour is more useful than a label. When a test fails,
you can read its name and know what broke without opening the file.

## Reading a failure

A failed assertion prints the values, then a stack trace pointing at the line.
The trace is there because tests are built in Debug by default, with safety
checks on. Those safety checks also let a test catch an out-of-bounds index
that a release build would let through.
...
real `zig test` output. It runs in your browser as WebAssembly.

The [testing](https://www.ziglang.in/learn/standard-library/testing/) chapter covers more: the
assertions that report what differed, and how to check that error paths clean
up after themselves.
