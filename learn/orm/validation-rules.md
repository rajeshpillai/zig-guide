...
A Zig library finds optional per-type behavior with `@hasDecl` at
compile time. A schema that declares `pub const rules = .{ ... }` gets
validation, and a schema without it gets no validation code at all.
...
Prefer declarations over registration functions for that reason. A
...
Zig users already know how to use it.
...
declarations following the same discovery pattern. The core stays the twenty
lines on this page. The extra features are added as more rules.
