...
Those lines contain three separate ideas:
...
Each of those ideas matches a Zig feature closely:
...
difference between the two designs is one thing only.
...
That one difference is where database behaviour lives. In Active Record, the
...
This design adds extra code between the pieces. In return, every piece
can be tested alone: a schema is a struct, a query is a
value you can inspect without running it, and the Repo is the single
seam you swap in a test. The [Repo chapter](https://www.ziglang.in/learn/orm/repo/) uses
that with a recording driver that never opens a connection.
Active Record is convenient. The cost is a test suite that needs a live
database even to check a validation rule.
