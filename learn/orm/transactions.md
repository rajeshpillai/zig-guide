...
In Zig, a transaction wrapper runs `BEGIN`, registers an `errdefer` that
runs `ROLLBACK`, runs the caller's work, then runs `COMMIT`. An error
anywhere after the `errdefer`, including in `COMMIT`, rolls the
transaction back and returns the original error.
...
the failed COMMIT itself. Zig's `errdefer` was designed for this case, and
a library API is a good place to use it.
...
Every `try` after the `errdefer` is covered by it. Success skips the `errdefer` entirely. There is no path that
