...
> The central pattern of a Zig library: take the caller's type, return a richer one.

The library these pages build is a miniature ORM in the style of Elixir's
Ecto. Each page takes one pattern and shrinks it to a snippet small enough to
verify. A full ORM adds drivers, pooling and migrations around these same basic
parts.

## The problem

A hard interface question for a Zig library is: how does the caller hand you
...
their build, with a message you wrote, naming their type. Those messages
are part of your API, so design them.

## The caller's type is not consumed

`Table` wraps the caller's type without changing it. `Users.Row` is the
caller's `User`, unchanged, so their code keeps constructing and passing plain
structs. This matters. If a library makes callers use *your* struct in place
of theirs, that struct spreads into every function signature they own.

## In a full ORM

This page's `Table` is the starting point for the real one. Grow it by adding
...
the query builder from the [next page](https://www.ziglang.in/learn/orm/query-builder/). Existing members
of the public API do not change. The API only gets new members.
