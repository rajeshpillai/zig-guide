...
Database migrations are easiest to test as data: an append-only list of
`{version, up, down}` entries, and a pure `plan(current, target)`
function that returns the steps to run. Executing the plan is a separate
loop, so every planning case is tested without a database.

## The problem

Schemas change after they ship. Most mature ORMs handle this the same way.
Each change is a versioned migration with an up and a down, the
list is append-only, and the database records which version it is at. The
design question is where the logic lives, because "compute
...
3. Executing the plan is a separate, simple loop that belongs to the
...
integers to a slice. The tests need no fixtures, containers or cleanup. When the planner
and the executor are one function, every one of those tests needs a live
database and most never get written.

The split also gives you `--dry-run` with no extra work: print the plan
instead of executing it. If your library can plan an operation without
performing it, users can review the plan before they run it.
