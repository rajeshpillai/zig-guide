...
only hold so many different patterns. One byte has 256 patterns. Nobody chose
this limit. It comes from counting. If you have eight switches, there are 256
ways to set them, and there is no 257th.

So a `u8` counts from 0 to 255. Then what?

A language can do one of three things here, and many well-known bugs come from
the difference between them. The value can wrap around to 0. The program can
stop. Or the operation can report "that did not fit" and let you decide. Zig
...
This is a large behavioural difference between the two languages. It is the
reason this material is easier to learn in Zig.
...
The first line silently wraps. The second is more dangerous. Signed overflow
is undefined behaviour in C. An optimiser is therefore allowed to assume it
never happens, and to delete the check you wrote to catch it. Beginners can
spend days on a bug like this, and many decide they are bad at programming.
