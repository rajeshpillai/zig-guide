...
Almost everything that feels mysterious later comes back to this. It explains
why a file and an image are the same thing to the operating system. It also
explains why a network can send numbers and receive text.
...
address, which is called **little-endian**. The hardware chose this order.
Arithmetic does not require it. Most machines you will meet do it this way. It
matters in exactly two
places, both much later: reading a file someone else wrote, and sending bytes
over a network.

**The last pair shows the same bytes read two ways.** `0x41424344` printed as the number
...
values. The bytes stayed where they were, and only the agreement changed.
...
The C version goes through a pointer. It will let you read five bytes, or four
bytes that were never there. `@bitCast` checks at compile time that the source
and destination are the same size, so `@bitCast` from a `u32` to a `[5]u8` does
not compile. You will see this pattern often: Zig lets you do the same
operation, but it blocks the mistake that usually comes with it.
