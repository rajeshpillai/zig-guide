...
Chapter one said everything is bytes. Text is where that fact starts to cost
you something.
...
letters you would count by eye. Someone built all of that for you. In Zig,
nobody built it.
...
**The type printed as `*const [5:0]u8`.** It reads: a pointer, to an array of
5 bytes, with a sentinel `0` after them. The type does not say "a string". It
describes a specific number of specific bytes at a specific place.
...
stored and `'h'` is a way of printing it. This is chapter one's point again,
in a place where it matters.

**Comparing needed `std.mem.eql`.** Zig will not let you write `a == b` on two
slices. The compiler rejects it: `operator == not allowed for type '[]const
u8'`. Two pointers do compare with `==`, and the answer is whether they point
at the same place. That is a valid question, but it is not the one you meant.
If you come from a managed language, make this a firm habit: **compare contents
with `std.mem.eql`.**
...
half of one. The correct version is to iterate codepoints with
...
The `strcpy` line is the one that has caused real damage. `strcpy` trusts a
