...
> Your program is handed streams it did not open, and must return a status.
...
The arrangement has three parts. It is older than almost everything else you
use, and it has not changed:
...
system. A shell relies on it to connect two programs together without either of
them agreeing in advance.
...
next free number. This is what the phrase "everything is a file" means in
practice. A file, a socket, a pipe and a terminal are all just numbers into that table,
...
land in the file while the stderr line still appears on your terminal. Tools
that mix the two up are painful to use from scripts.
...
One small integer is not much to say with, so programs also write to stderr.
The status says *that* it failed. stderr says *what* happened.
...
Because the page runs it under **WASI**, an interface with the same shape as
the real thing: the same numbering, the same calls, written in JavaScript
against the browser instead of against a kernel.

The program cannot tell the difference. An interface is exactly that promise,
and most of the snippets in this guide run on a page because of it. Three
chapters in [the OS section](https://www.ziglang.in/learn/os/) need a real kernel: pipes,
signals and spawning. They say so, and they do not offer a Run button.

## Where to go next

That is the end of the track. You now have the model the rest of the guide
assumes.
