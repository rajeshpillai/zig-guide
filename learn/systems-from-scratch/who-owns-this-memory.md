...
So somebody has to be responsible for handing it back. Memory management comes
down to one question: **who owns this, and when do they give it
...
The last part is what makes Zig different. In Zig, **a function that
...
tests it is a failed build: `std.testing.allocator` fails any test that leaks.
...
The report tells you that a rule was broken. It does not tell you what the fix
should be.
