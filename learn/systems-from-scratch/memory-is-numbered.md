...
one end. The number of a box is its **address**.
...
address. The rest of what you learn about pointers follows from this.
...
does not change is the **spacing** between the addresses.
...
directly against each other with no gap. Because of this, `row[2]` can be
found without searching for it: the machine computes the address of `row[0]` plus
two times four, and goes straight there. Finding element 999,999 of a large
array takes the same arithmetic as finding element 0, which is not true of
every data structure you will meet.
...
Zig to let you look at it as one. Zig makes you ask because once you can do
arithmetic on an address, you can also get that arithmetic wrong.

**The pointer was 4 bytes wide.** This page runs your program
...
That width is what "64-bit" describes.

**`p.* = 99` changed `row[2]` itself, not a copy of it.** The pointer held the
...
addresses did not become smaller. The boxes did, so more of them fit into the
...
travels, is in [Pointers](https://www.ziglang.in/learn/language-basics/pointers/).
