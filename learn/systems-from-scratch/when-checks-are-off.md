...
The last chapter ended on an awkward fact: the program you ran said its safety
checks were absent. The reason they were absent is what separates systems
programming from everything above it.
...
In a systems language, some questions are left unanswered on purpose. That
does not mean "answered unpredictably". It means **unanswered**. The
specification says that if your program does this thing, the language makes no
promise about what happens next. That is **undefined behaviour**. It is easy to
misread it as a fancy way of saying "crash" or "garbage value". It is neither.
...
could only be true in a situation you promised would not arise. So undefined
behaviour is worse than a wrong answer. A wrong answer stays in one place. A
broken promise can change code somewhere else.

Zig takes this approach: **the checks exist, and you choose per build whether
to pay for them.**
...
**The program read its own build mode.** `builtin.optimize` is available to any Zig
program, and `builtin.optimize.runtimeSafety()` is the direct question: are the
checks in this build. Printing them is a useful habit. Almost every bug of the
kind "why does it behave differently on the server" comes back to this.
...
- **ReleaseSafe**: optimised, and the safety checks stay. This is the mode most
  server software should ship in. The checks cost something, but not much. A
  program that stops with a clear message is better than one that continues
  with corrupt state.
...
by every reader. The output above says `small` and `false` for that reason.

**The four operators gave the same result in every mode.** `+%`,
`std.math.add`, `@addWithOverflow` and `+|` behave identically in all four
modes, because each of them states in the source what should happen when the
value does not fit. They do not rely on a check that a release build is allowed
to remove. When overflow matters, say in the source what you want. Then the
build mode does not change the answer.

## The one that does depend on the build

Here are the same two values as the four lines above, added with a plain `+`.
It is the one snippet on this page built ReleaseSafe rather than ReleaseSmall,
so the check is still in it. Press **Run**.
...
Now compare that with the four operators above. Each of them
...
The last two rows do **not** say "wraps". It might wrap on your machine today.
Writing code that depends on that is exactly the broken promise described at
the top of this page. If you want wrapping, use `+%`. It costs nothing.
...
check inside a loop running a billion times per frame is measurable, and in
some programs that speed is what matters most. The judgement is per program,
and sometimes per function. For this reason Zig lets you turn safety off for a single
