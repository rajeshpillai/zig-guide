...
What is at the address just past it? Something is always there: another
variable, part of a frame, or the machinery of the program itself. Memory keeps
going past the end of your array. Only your right to use it ends there.
...
asked, by making the caller check first. The third needs nothing from the
language except an `if`, so that is where to start.
...
together as one value, so a function receiving one always knows the size. This
is why `[]const u32`, and not a bare pointer, is the normal way to pass a
sequence around. The pointer alone would have told the function where the data starts
...
result. All of that cost one comparison.
...
The run also says it cannot print a stack trace, because debug info is
unavailable for this target. The wasm sent to your browser carries no symbols,
so there is nothing to turn a return address into a file and a line. On your
own machine the same panic comes with the chain of calls that led to it.
...
cost, and it is the reason the rest of the site does not carry them. It
...
This is a difference you will meet early.
