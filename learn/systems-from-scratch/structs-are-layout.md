...
on some processors it is not allowed at all. So the rule for a number is that
it starts at an address divisible by its own size.

Apply that rule to a struct whose fields are different sizes, and you get a
result you may not expect: **the struct is bigger than its fields**. The gaps inserted to keep each field aligned are called **padding**,
...
ends at 16. The struct's own alignment also rises to 8, because a struct takes
the alignment of its most demanding field, which is what makes arrays of it
work. Adding one 8-byte field to an 8-byte struct cost 8 bytes, and that is the
ordinary case rather than a surprise.

## If you have written C

The rules, the padding and the arithmetic are the same. There are two
differences.
...
and `packed struct`. For a plain struct, the compiler decides. That is the
...
and needs no include. The program above used it to show the gaps, so you do
