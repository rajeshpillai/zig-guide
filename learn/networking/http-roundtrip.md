...
Zig's standard library has both halves of HTTP with no dependencies:
`std.http.Server` answers requests over any reader and writer, and
`std.http.Client.fetch` makes a request in one call. Both are built on
the `std.Io` reader and writer interfaces.

## The problem

Both halves were rebuilt on the `std.Io` interfaces. Most example code you
will find for either half was written before that rebuild.

As with [TCP](https://www.ziglang.in/learn/networking/tcp-echo/), the recipe runs server and client
in one process. CI runs it and compares the output, so the page cannot go out
of date without CI failing.

## The plan

1. Listen on an ephemeral TCP port, exactly as in the TCP recipe. HTTP is
   a layer on top of a TCP socket. It does not need a different kind of
   socket.
...
two interfaces that every stream in current Zig exposes. The HTTP layer never
sees the socket. Give it a pipe or a fixed buffer and it works the same way.
The standard library tests it like that. If you understand the TCP recipe, the
only new machinery here is head parsing and response formatting, and both
belong to `std.http`.

The server loop also shows keep-alive in action. Both fetches in the snippet
arrive over one TCP connection, as two iterations of the `receiveHead` loop. The loop exits when the client's connection pool closes the stream.
...
headers, or long-lived requests, use `client.request()` instead. It does the
same steps, but you write each one yourself.

## Status is data, not an error

The 404 comes back as `FetchResult.status`, not as a Zig error. Errors are for
the transport failing (refused connection, malformed response). Your
application decides whether a 404 is a failure. Route on the status enum: `.ok`,
