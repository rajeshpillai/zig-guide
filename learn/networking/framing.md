...
SMTP, IMAP and Redis all use it for their text parts. You can debug it with `nc` by reading the bytes.

The cost is that this byte can never appear in a payload. The last line
...
readability, use a length prefix.

If you want it debuggable by hand, use a delimiter for the envelope and a
length for the body. HTTP has used that combination for thirty years.
