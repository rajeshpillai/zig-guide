...
In current Zig, a TCP server is `std.Io.net.IpAddress.parse` followed by
`address.listen(io, .{})` and `server.accept(io)`, and a client is
`addr.connect(io, .{ .mode = .stream })`. A connected stream's reader is
the same `std.Io.Reader` you use for files.

## The problem

You want a TCP server and a client talking to it, in Zig, with today's API.
Out-of-date tutorials cause a lot of trouble on this topic. Networking moved from
`std.net` to `std.Io.net`, and every operation now takes the `Io` instance.
Code copied from a guide written last year does not get past the compiler.

The recipe runs both halves in one process. This is also how you write an integration test for any protocol
code.
...
does not know that a socket is underneath, which is what makes it
testable with `std.Io.Reader.fixed` in unit tests.

## Why a thread

`accept` blocks until a client arrives, and `connect` blocks until the server
accepts, so one thread cannot do both halves interleaved without relying on details of the kernel's
connection queue. A second thread keeps each half straight-line. On the
...
- **Structured protocols:** length-prefixed frames work better than line-delimited text for binary data; see [the wire format recipe](https://www.ziglang.in/learn/how-to/binary-roundtrip/).
