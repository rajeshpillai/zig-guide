...
machines go away, and a connection ends every single time it succeeds. These things will happen. What matters is whether your code can tell them
apart.
...
client that ever behaves correctly, and the log becomes useless.
...
> in a length 0 return value, in which case `error.EndOfStream` is returned
> instead.
...
`takeDelimiterInclusive` returns the delimiter as part of the slice, so it cannot make one up. No delimiter means `EndOfStream`, and you
