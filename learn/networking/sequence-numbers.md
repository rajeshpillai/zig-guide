...
A sequence number on every message lets a receiver detect both loss and
duplicates, and a cumulative ack ("I have everything through N") tells
the sender what it has to resend. TCP's acks follow the same design.
...
expects 2 and gets 3 now knows a message is missing, although the network never said so. A
receiver that gets 4 twice knows the second one is a retry, not a new message.
...
of the stream it has. That reply hands the loss to the sender, and only the sender can fix it.
...
The `Watcher` is a single `u8` and a comparison, and that is all sequence-number detection needs. Everything it reports in the first
half of the trace follows from one rule: numbers arrive in order unless
the network interfered. The duplicate of 4 is caught by the same comparison that catches gaps. The
last chapter said retries create duplicates. This receiver already handles
them, one chapter before retries appear.
...
through 1", even while 3, 4 and 5 are arriving. That wording has two effects. First, acks repair themselves: if one ack is lost, the
next arrival repeats everything it said. Second, the sender only has to track one number. Look at the final line of the
trace: the sender heard "through 1" and concluded that 2 through 5 need
resending. It never saw the drop. It worked out what was lost from the replies alone. That is TCP's ack design, and this snippet's version
...
In this snippet the acks always arrive; this is the one simplification, made to keep the trace readable. On a real network the ack for 2 can vanish
...
- **Selective acks:** "I have 0 through 1, and also 3 through 5" names
  ...
  as the SACK option fifteen years after cumulative acks shipped.
