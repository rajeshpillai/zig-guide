...
connection costs a stack each and stops scaling in the thousands. A single-threaded event loop scales, but it forces callbacks into every
function you write.
...
never learns where they came from, so this page runs with no network
...
This is the reason to write protocol code against `Reader` and `Writer`
instead of against a socket. The tests are fast, and the browser can run the
example.
...
[Threads](https://www.ziglang.in/learn/concurrency/threads/) is a page you cannot run here and
this one is.

If you are arriving from an older Zig looking for `async` and `await` as
keywords: they were removed. `io.async` replaced them, as a library rather
than syntax. [Concurrency](https://www.ziglang.in/learn/concurrency/async-future-group/) covers the
shape in general.
...
[Cancellation](https://www.ziglang.in/learn/concurrency/cancellation/) covers what a handler
...
that connection's reply, the same conclusion the [text protocol
...
The single-threaded implementation this page runs on would work without the mutex. Do not rely on that. Code that is only correct
under the implementation you tested with has a data race that can appear once
it is deployed, because the `Io` you get in production is the threaded one.
...
The `COUNT` values are the exception. They show the order the tasks actually
reached the mutex. Run this natively and they may come out in a different
order.
