...
To call Zig from JavaScript, mark the function `export fn`, build for
`wasm32-freestanding` with `-fno-entry` and `--export=<name>`, and call it as
`instance.exports.<name>` once `WebAssembly.instantiate` has run.

## The problem

Typical uses are a hot loop, a parser, or some math the JavaScript engine is
slow at. The module has no `main`. JavaScript instantiates it
...
name. There is no name mangling and no wrapper. `add` in Zig is `add` in the export table.
...
- **Naming exports differently:** when the Zig name and the name the host should
  see differ, use `@export(&foo, .{ .name = "bar" })` and give `foo` a
  `callconv(.c)`, since `@export` does not add one the way `export fn` does.
