...
To load a Zig wasm module in the browser, pass `fetch("module.wasm")` to
`WebAssembly.instantiateStreaming` along with an imports object, then call the
functions on `instance.exports`. The module calls back into JavaScript through
`extern` functions that the imports object supplies by name.
...
to. This site does the same on every Run button. This page shows all the parts
together.
...
Rename either side and it stops resolving. The engine checks this interface at
instantiation.
...
There is a small circular dependency here. The import `log` needs `memory` to read
...
above, is the usual way to solve this.
...
- **This site, exactly:** the playground fetches the verified `.wasm`, builds a
  WASI import object instead of a single `log`, and calls `_start`. The
  imports differ, but the mechanism is the same.
