...
To run a Zig program as WebAssembly outside the browser, build it with `zig
build-exe -target wasm32-wasi` and run the `.wasm` under a WASI runtime such
as `wasmtime`, `wasmer` or Node's `node:wasi`. The program itself needs no
wasm-specific code.

## The problem

You have a Zig program with a `main`, and you want a single `.wasm` that runs
the same on Linux, macOS, and Windows, without recompiling. WASI makes that
possible. It is the system interface a wasm command calls for the OS-shaped
things (stdout here) that pure wasm cannot do.
...
receives. The code has no wasm-specific setup.
...
(`tools/run-wasi.mjs` in this repo) is about a hundred lines: it opens the
module, builds a WASI import object, and calls `_start`.

## What WASI is doing

The program prints through a buffered writer over stdout. On a native build
the flush lowers to a `write` syscall. On `wasm32-wasi` it lowers to a call to
...
The runtime decides which capabilities to grant. A runtime can refuse to preopen any
...
- **Arguments and env:** WASI passes `argv` and environment variables when the
  runtime chooses to. You read them off the `std.process.Init` that `main`
  receives, the same way as on native. On WASI the argument iterator needs an
  allocator, so use `Args.Iterator.initAllocator`.
- **Files:** `std.Io.Dir` works, but only inside directories the runtime
  preopened. Nothing else on disk is reachable, by construction.
