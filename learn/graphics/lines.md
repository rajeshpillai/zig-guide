...
Keeping `dy` negative collapses the eight cases (four quadrants, shallow and steep)
...
The count holds only because the error term is symmetric. Check it, because a
renderer that draws shared edges from both directions will otherwise leave
seams.
...
is fine at this size and wasteful at larger ones; the Cohen-Sutherland
