...
general; here the benefit is easy to see.
...
just comes out slightly smeared in the direction of the scan, so code review
often misses this bug.

Image pipelines either keep two buffers and swap them, or write to a fresh one
each pass. The three-pass blur in the snippet does exactly that.
...
difference it already found. On a hard edge, that overshoot is called ringing.
A sharpen slider trades sharpness against ringing, and pushing one far enough
always produces a halo.
...
Separability saves more. The gaussian above factors into a horizontal
