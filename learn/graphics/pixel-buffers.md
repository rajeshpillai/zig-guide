...
- **Top-left origin.** `y = 0` is the top row and `y` grows downward. Nearly
  ...
  you learned in maths. You will probably get it wrong at least once.
...
declared as a local may work on one machine and overflow the stack on another.
Make it global, or heap-allocate it.

`undefined` is safe here only because `drawGradient` writes every cell
...
but the last, because integer division truncates. This is a common bug
in integer scaling code, and it is invisible until you look at the output.

**`width - 1`, not `width`.** With `width` the last column stops short: at 64
wide it reaches 251, not 255. Nobody sees that in a gradient. It matters the
moment the value is used as an index.

No floats appear anywhere in this section unless a chapter says otherwise.
Integers are exact for this work, and exact values help a lot when you are
...
wrong answer everywhere else. Coordinates that can go negative need a signed
type, and the cost is an `@intCast` where the value is used.
...
[Circles](../circles/) is a handful of lines even when it hangs off two
...
dominate the cost. It is fine for learning, but too slow for a hot loop.
