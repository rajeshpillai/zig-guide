...
> PPM, the image format you can write without a library, and the one strict rule that is easy to break.

A buffer becomes useful as a picture once you can look at it. PPM is the
shortest way to save one: a short text header, then the pixel bytes. There is
nothing more to the format, and it needs no library at all.
...
text, which is larger and slower but useful to generate once if you want to read
...
- **maxval from 256 to 65535** gives *two* bytes per sample, **big-endian**.
  This is the only place PPM has a byte order, and it causes bugs on
...
the program deletes the file before you can look at it.
