...
**Going past 1 is not an error.** The right-hand square asks for
...
It comes from the limit of what an affine map can represent. A triangle's
...
The seam is the same defect as perspective in 3D, seen in 2D where it is easier to
...
pixel. "Perspective correct" names that fix, and it was too expensive for much
of the hardware of that time.

## Why a texture is not a canvas

`Texture` here is its own type, with `pixels`, `width` and `height` and no
`stride`. The missing `stride` is a deliberate difference from [the
canvas](https://www.ziglang.in/learn/graphics/sub-canvas/). Nothing in this program samples a
rectangle out of the middle of a larger image, so there is no second width to
carry.

The separate type gives a guarantee. `Texture` has no `set`, and its pixels are
`[]const u32`, so the type says which image is read and which is written.
...
The clamp catches it and returns texel 7. The branch reads `@min(w - 1, tx)`
for that reason, not `@min(w, tx)`.

No pixel in this program actually reaches it. `u` is exactly 1 only on the
line through the other two corners, where the weight of the `u = 0` corner
falls to 0, and for the left square that line is `x = 26`. Pixel centres sit
at half-integers, so they miss it by half a pixel every time.

So the guard is never exercised by the picture above, and nothing here would
notice if it were wrong. The snippet you are looking at also ships with the
bounds check compiled out. Move one vertex to a half-integer and the read goes
off the end of the texture. A safety build catches that with a panic. This one
returns a pixel of whatever follows the texture in memory.

## If you have written C

The rasteriser is the shape you would write in C, with one difference: `@mod`
rather than `%`.
...
The symptom is a single wrong row or column along one edge, easy to mistake
for an off-by-one in the geometry.
...
the divisor, the wrapping a sampler wants. Both exist and neither is the
default, so you must choose one.
