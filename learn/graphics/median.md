...
The output confirms the morphology at the end: the block covers 153
...
data-dependent branches at all. That matters because a mispredicted branch per
pixel is expensive, and a network has none. It is also directly vectorisable,
which the loop above is not.

The size of the network depends on what you ask for. Sorting nine elements
takes 25 compare-exchanges, but the median alone takes 19, because a selection
network may leave the rest of the window in any order it likes. You only need
the middle value, so you only pay for the middle value.
