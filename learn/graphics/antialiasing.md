...
> Coverage is alpha. Sample the inside test sixteen times per pixel and the jagged edges go away.
...
(dropping it) biases the whole edge by half a sub-pixel, an eighth of a pixel
here, which is small, systematic, and visible when two antialiased shapes are
meant to meet.

## What this costs

Sixteen inside tests per pixel, over a bounding box that has to grow by one
pixel on each side so partially covered pixels are not skipped. This cost is
why supersampling is often the first technique people use, and the one they
replace later.
...
evaluating it more often. So it is a good first choice, and it can stay the
answer for offline rendering.
