...
simply skipped the cells outside the canvas. No extra code does the clipping:
it comes from the bounds check.

**The sprite is stored once and drawn three times.** Nothing was duplicated in
memory. A sprite is
...
pixel becomes a blend of source and destination weighted by alpha. The
[blending](https://www.ziglang.in/learn/graphics/blending/) chapter builds exactly that, which is why
it comes before this one.
...
there is no transparency, because `memcpy` cannot skip. This trade-off shapes
all of 2D blitting: opaque rectangles are a fast row copy, and
anything with holes in it is a per-pixel loop.

Hardware sprite units existed for this reason. The machines that made this
