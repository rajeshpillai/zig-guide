...
**Two lines changed and four shapes moved.** `nodes[0]` went from (4, 3) to (26, 5).
...
down the tree. The layout engine uses the same downward walk, for
...
Two integers only allow moving. A full scene graph stores a **matrix** per
...
Matrices give scene graphs their main benefit. A
...
Usually not, and caching has its own cost. Caching each node's
...
contiguous memory. Contiguous memory makes the walk faster, and that counts
when the walk happens sixty times a second.
