...
> Count what the image contains, and the lookup table that fixes it comes straight from the counts. Auto-levels, equalization, and why you apply them to luma.
...
Two hundred and fifty six counters and one pass. The analysis needs nothing
more, and everything else on this page is derived from these counts.
...
background ramp lives between 13 and 68, and the block and discs are three
...
Clipping a fraction at each end first is deliberate. A single hot pixel at 255, or one dead pixel at 0, would otherwise set
...
The change in the output is large: the background ramp, which was three
...
Equalization redistributes values; it cannot invent them. If 153 pixels share
...
Equalization also amplifies whatever is in the sparse regions, including
noise. A flat sky with sensor grain becomes a
flat sky with visible sensor grain, because the algorithm cannot tell the
difference between detail that should be stretched and noise that should be
left alone. This
...
Use per-channel when the colour cast is the problem. Use luma when the colours
