...
Off by one per channel is below what anyone can see, and the two-pass version
is the closer of the two. Composing truncates the nine matrix entries
themselves, and each of those errors is then multiplied by a channel value as
large as 255. Rounding twice on the way through costs less than rounding the
matrix once. It matters because "same filter, different order of operations"
