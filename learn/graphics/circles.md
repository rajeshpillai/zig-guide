...
This trick is useful well beyond graphics: any time you are about
...
`fillCircle` checks for that. The last circle in the snippet hangs off two
...
Inside the outer radius and outside the inner one is a band. It needs no new
algorithm or separate case, and the thickness is a parameter. The midpoint circle
algorithm draws a one-pixel outline faster by walking only one octant and
mirroring it eight ways, which is the circle equivalent of Bresenham. Read it
once. For anything you intend to fill, though, the squared-distance test is
simpler and vectorizes.
...
3. **A span fill.** For each row, solve for the two x values where the circle
   crosses it and write that run of pixels directly. One `@memset` per row is much
   faster than a per-pixel test, and production rasterizers work this way.
