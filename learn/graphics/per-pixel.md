...
> Brightness, contrast, threshold and invert are all one table lookup, and brightness needs Zig's saturating add.
...
operations, and they share a useful property. On 8-bit channels there are
only 256 possible inputs, so there are only 256 possible answers. Compute them
once into an array, and the filter becomes an array index.
...
That loop runs every filter of this kind. Brightness and threshold use the
same code and differ only in 256 bytes of data.
...
## Brightness needs a saturating add
...
in a brightened sky. It is the default in C, where unsigned overflow is
defined to wrap and nothing warns. The first is what most people want as a
default, but no language gives it to them, because there is no sensible value
to return.

`+|` and `-|` are language operators, and they compile to a single
instruction on every target that has one. [Integer
...
operation cannot help with a value that was never a `u8`. So the code widens
to `i32` first, clamps to 0..255, and only then narrows back to `u8`. Any
other order gives wrong results.
...
difference. Every step maps bytes to bytes, so composing adds no error.
...
So editing pipelines that care about quality keep more than 8 bits per
channel until the last step. At 8 bits every filter in the chain rounds, and the
