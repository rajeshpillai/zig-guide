...
A production pipeline usually does not normalize. It rejects one winding on
purpose. That is *backface culling*, and it is how a renderer cheaply discards
the far side of a closed mesh, before touching a single pixel.
...
coordinates, normals, and depth. This is the main reason edge functions replaced
scanline approaches: the inside test and the attribute interpolation are
the same arithmetic.
