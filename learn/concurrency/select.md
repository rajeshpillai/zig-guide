...
`select.async` exists and matches `Group.async`, but arms that are allowed to
run inline do not really race. On an `Io` that runs tasks inline,
...
machine is, so the expected output holds on CI. With a margin of
milliseconds, the winner could change from run to run, and the test would
sometimes fail.
