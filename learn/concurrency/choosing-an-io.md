...
Every other chapter in this section takes `init.io` and passes it down. This
one looks at where that value comes from. The choice belongs to the program,
and `main` is the only place in a codebase that has to make it.
...
Two options are useful. `async_limit` caps the pool used by `io.async`;
...
It is also a good way to test that library code still works correctly with no
threads. If your
...
This is the benefit of the `Io` parameter. A server written against the interface
runs on a thread pool during development and on io_uring in production without
a line changing, and the decision that picks between them stays in `main`.
...
ask for concurrency. Then the choice on this page is the caller's. That is why blocking goes
through an interface.

`Io.Threaded.global_single_threaded` exists as a fallback for debugging
and for code that really cannot take a parameter. Using it in a library takes
the choice of implementation away from the caller.
