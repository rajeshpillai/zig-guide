...
In Zig, `std.Thread.spawn` starts a real OS thread and `join` waits for it to
finish. Shared data is guarded with `std.Io.Mutex`, because `std.Thread.Mutex`
no longer exists.
...
This is the change that breaks older code. On `std.Io.Mutex`, `lock` takes the
io instance:
...
`lock` can fail only because it can be *cancelled*, and that is what makes it
compose with the rest of the `Io` machinery.
See [Cancellation](https://www.ziglang.in/learn/concurrency/cancellation/) for what that means.

The move changes more than the name. A mutex that knows about the `Io` instance can be
...
released on every path out. `defer` handles the second. Nothing checks the
first for you. So aim for a design with the smallest possible amount of shared
state.
...
That second argument is a memory ordering, and it constrains the memory
around the operation rather than the counter itself. `.monotonic` is right
for a total nobody reads until the end, and wrong for a flag that publishes
data written before it. [Atomics](https://www.ziglang.in/learn/concurrency/atomics/) covers the six
orderings, the compare-and-swap loop that every operation without a dedicated
`fetch` is built from, and the working mutex `std.atomic` assembles out of
two of them.

## Prefer structured concurrency

For most work, [`Io.async` and
`Io.Group`](https://www.ziglang.in/learn/concurrency/async-future-group/) are a better fit than
...
A thread is a low-level resource, in the same way a raw allocation is. Most code wants "run these things concurrently and wait for them",
which is a scope with a lifetime, not a handle to manage. Reach for
`std.Thread.spawn` when you really want a long-lived worker with its own
