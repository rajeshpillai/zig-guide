...
capacity, its lifetime and where it lives are all yours to decide, just as
with `Io.Writer`. A queue on the stack of the function that spawns both
...
The consumer never has to be told how many elements are coming, so a producer
can decide how many to send as it goes.
...
queue blocking the sender *is* the back pressure, and the pair runs at the
speed of the slower side.

This is the case that needs
[`io.concurrent`](https://www.ziglang.in/learn/concurrency/async-future-group/) rather than
...
still waiting for `async` to return. The two spellings exist because of
this deadlock. It is also why this snippet is native and the one at the top
of the page is not.
...
[Cancellation](https://www.ziglang.in/learn/concurrency/cancellation/); the rule that
