...
`Io.Mutex` covers most shared state. `RwLock`, `Semaphore` and `Condition`
are what you reach for past that point, and each takes the `Io` instance for
...
cannot join a writer. Use it only when reads are much more common than writes
and hold the lock long enough to matter. An `RwLock`
