# Async, Future and Group

> async, Future, and Group, all through the Io interface.

Zig's concurrency now runs through `std.Io`. That interface is the
subject of [The Io Interface](https://www.ziglang.in/learn/standard-library/io-interface/), and this
section assumes it: every chapter here takes an `Io` and passes it down.

Press Run: this executes in your browser, on a target with **no threads at
all**.
...
code does not change**, and that is why this page is runnable and the
[Threads](https://www.ziglang.in/learn/concurrency/threads/) page is not.
...
the program fails on the single-threaded targets where `async` still works.
...
property is what "structured" means here. Raw `Thread.spawn` does not give
you this.
...
chapter](https://www.ziglang.in/learn/concurrency/cancellation/).

## The bigger picture

`Io` works the way `Allocator` does. A function that takes an `Allocator`
lets the caller choose where memory comes from. A function that takes an `Io`
announces that it may block, and the caller
...
The other chapters in this section build on this.
[Queues](https://www.ziglang.in/learn/concurrency/queues/) for tasks that hand work to each
other. [Select](https://www.ziglang.in/learn/concurrency/select/) for whichever finishes first.
[Locks](https://www.ziglang.in/learn/concurrency/locks/) for the shared state underneath. And
[Choosing an Io](https://www.ziglang.in/learn/concurrency/choosing-an-io/) for where the
