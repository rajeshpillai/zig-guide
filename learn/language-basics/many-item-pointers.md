...
In Zig, `[*]T` is a many-item pointer: it points at an unknown number of `T`
and supports indexing, but has no `.len` and no bounds checking. Slicing it
with `many[0..count]` gives a checked `[]T` once the length is known.
...
The split has a purpose. In C, a function signature taking a `char *`
...
That one step gives you back bounds checks, `for` iteration, and `.len`. Prefer
slices everywhere except the boundary where a length is not available.
...
After that line, everything is checked again. So the unchecked assertion sits
in one visible line, not in every access.
...
C string, and it is the type translate-c gives you for `char *` parameters
that are documented as null-terminated.
...
- **From C.** Every array parameter in a C header is a pointer, so translate-c
...
zero is legitimately mapped. You only need it on those targets.
