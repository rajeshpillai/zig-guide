...
In Zig, `|value|` after `if`, `while`, `for`, `switch` or `catch` names the
unwrapped payload: the non-null value, the success value, the active variant
or the current element. `|*value|` captures a pointer instead, so writes reach
the original.
