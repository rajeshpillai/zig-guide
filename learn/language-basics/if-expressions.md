...
converts to one. There is no ternary `?:` either. An `if` used as an
expression does that job: `const x = if (a) 1 else 2;`.
...
Both branches must agree on a type. When `if` is used as an expression, the
`else` is required. Without it, the missing branch produces `void`, and the
error names that type:
...
`if` on an optional is the general form. Three shorter forms cover the common
cases:
