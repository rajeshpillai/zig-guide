...
In Zig, `switch` can match single values, lists such as `10, 100` and
inclusive ranges such as `-1...1`, and it can be used as an expression whose
value goes straight into a `const`.
...
forgetting one. An arm that should cover several values lists them. In C,
that was the usual reason to use fallthrough.
...
ranges, which excludes the upper bound. The difference is deliberate, and it
is easy to get wrong: `0..3` in a `for` visits three values, `0...3` in a `switch`
matches four.

Cases must not overlap. Two arms that can both match the same value are a
compile error. There is no first-match-wins rule, so the order the arms are
...
silently classify the new direction as horizontal. With an `else` branch, the
compiler can no longer tell you about a new tag. Use `else` only when that is
what you want.
...
Two more forms help once you are generating code instead of writing it out.
...
## A switch can be labelled

Give a switch a label and `continue :label value` runs it again with a new
operand, while `break :label value` leaves it with a result. That turns a
switch into a state machine with no loop around it. It has its own chapter:
[labelled switch](https://www.ziglang.in/learn/language-basics/labelled-switch/).
