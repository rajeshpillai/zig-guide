...
Assigning an array copies it. This is the opposite of C, where an array
turns into a pointer in almost every expression. If you want a reference, take one
...
Because an array is a value, passing a `[4096]u8` to a function passes 4096
bytes, or at least gives the compiler the option. Passing `&buffer` passes a
slice, which is a pointer and a length. For anything larger than a few words,
pass the slice.

## Writing one down

`[_]T` lets the compiler count for you. Prefer this form, because the length
cannot fall out of sync with the contents:
...
The `**` operator has been **removed**. `**` is no longer even a token, so the
...
Array to slice is the usual direction. Build with an array, where the size is known and
...
array, needs the length to be comptime-known. It is covered in [array
recipes](https://www.ziglang.in/learn/language-basics/array-recipes/), along with grids and tables
