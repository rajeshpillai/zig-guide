...
Zig has anonymous structs in two forms: a struct type written inline where it
is used, and a tuple, which is a struct whose fields are numbered rather than
named. The argument list passed to `print` is a tuple.
...
This is useful for a one-off shape that does not need a name, and for
returning several values at once without declaring a type for the pair. Two
separately written anonymous struct types are distinct types even if their
fields match. So use them for local convenience, not for types that cross an
interface.
...
The numbers are the field names. The first field is called `0`, the second
`1`, and so on. A name cannot start with a digit, so `tuple.1` does not
parse. Wrap the name in `@""` instead: `tuple.@"1"` is ordinary field access,
the same escape Zig uses for any name that is not a plain identifier.
`tuple[1]` is a shorter way to write it. When the name is a string you built
at compile time, `@field(tuple, "1")` reads the same field.
...
type of the expression depends on a runtime value, and Zig does not allow that.
...
`{d}` is a compile error, not a runtime error.

Because the arguments are a tuple, several rules follow. Passing the wrong number of
...
known. Zig has no `printf` format-string vulnerability, because the format
string and the arguments are checked against each other before the
program runs.

The tuple also explains the empty case. A call with no arguments still passes a
