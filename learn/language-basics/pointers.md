...
somewhere else or in a sentinel. See [slices](https://www.ziglang.in/learn/language-basics/slices/)
and [many-item pointers](https://www.ziglang.in/learn/language-basics/many-item-pointers/).

## Never null

There is no null `*T`. If a pointer may be absent, its type says so: `?*T`.
The compiler enforces this. It will not let you dereference the
optional without unwrapping it first.

The representation costs nothing. Zig uses the null address as the `null` tag,
so `?*T` is the same size as `*T`.
...
Both exist for the cases that really need an integer: talking to hardware,
...
[the stack](https://www.ziglang.in/learn/systems-from-scratch/the-stack/).
