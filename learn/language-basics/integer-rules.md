...
Zig integer types carry an explicit width, such as `u8`, `i32` or `u3`, and
narrowing from a wider type needs `@intCast`. Plain `+` treats overflow as
illegal behaviour, which panics in safety builds; `+%` wraps and `+|`
saturates when that is what you want.
...
the width that matches a byte. Packed structs use this to describe
bitfields exactly.

The width is part of the type and never depends on the platform. Zig has no
`int` whose size changes with the compiler, and no integer promotion rules to
remember. Your machine and the CI machine compute the same results. The one target-dependent integer type is `usize`, and it is
...
it needs `@intCast` too. `@bitCast` is the other conversion. It keeps the
same bits and reads them as a different type. It never checks anything,
because nothing can go wrong at the bit level. Use `@intCast` when you mean
"this number" and `@bitCast` when you mean "these bits".
...
In C, overflow can corrupt a value silently. In Zig, the behaviour is an
explicit choice.
...
else. A number that no longer fits its type means there is a bug. It is
better to find out at once than three functions later.

A common mistake is unsigned subtraction. `a - b` where `b > a` is
...
`/` on a signed integer that is not comptime-known does not compile at all.
Rounding towards zero and rounding down disagree the moment an operand is
negative, so Zig makes you name the one you meant: `@divTrunc`, `@divFloor`,
`@divCeil` or `@divExact`. `%` is the same, and pairs with `@rem` and `@mod`.
C shows why the choice matters: its answer to `-7 / 2` was
implementation-defined for twenty years.

Shifting by more than the width is illegal behaviour on every platform. The shift amount's type is also narrowed to just enough bits to
...
`comptime_int` has arbitrary precision, unlike `comptime_float`,
...
somewhere with a size. For this reason a `var` cannot hold a `comptime_int`,
and
