...
Zig's counting loop is `while (i < n) : (i += 1)`, where the continue
expression after the colon runs after every iteration, including ones cut
short by `continue`.
...
by `continue`. That is why the continue expression exists, instead of just
putting the increment at the end of the body. Once `continue` is involved the two are not
