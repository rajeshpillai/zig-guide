...
Use `@memcpy` when you know the ranges are disjoint, and `@memmove` when they
might not be.
...
`orderedRemove` does for you on an `ArrayList`. Here it is without the
container.
...
`std.mem.findMin` and `findMax` return **indices**, not values. That looks
odd at first. It makes sense when you have a second array of labels lined up
with the first, which is the case they are built for. `std.mem.allEqual` answers the
...
narrow afterwards. This mistake is easy to make with every function in the
`find` family once you work with sub-slices.
