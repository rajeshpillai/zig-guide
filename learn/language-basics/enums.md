...
A Zig enum is a type whose value is exactly one of a fixed set of named tags.
An enum can pin its integer tag type with `enum(u8)`, declare methods, and
accept values outside the named tags when a trailing `_` makes it
non-exhaustive.
...
literal type, which coerces to any enum that has a matching tag. So the same
`.north` works for two unrelated enums in two different calls.
...
the values are still ordered by declaration, but do not write either into a
file format. Pin the type when the numbers leave your program.
...
where you do not control the set of values. `@enumFromInt` on one of these
...
`Suit.isRed(Suit.hearts)`: method syntax is a shorter way to pass the receiver
as the first argument. The call goes straight to that function, with no
dynamic dispatch.
...
between `0.17.0-dev.644` and `dev.1441`, and was the only break anywhere in
this guide when the compiler was updated.

`@tagName` gives the name as a string. With `@tagName` and these arrays,
generic code prints or parses enums without a hand-written table. Walk `field_names` with
