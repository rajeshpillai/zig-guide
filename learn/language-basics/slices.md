...
Lifetime is the one hazard the type does not check. The slice will not stop you
indexing it, because as far as the type is concerned it still has a valid
pointer and a valid length. What changed is that the memory it points at now
belongs to something else. Returning a slice
...
The bounds themselves *are* checked. Write `array[1..10]` on a three-element
array with both bounds comptime-known and the compiler rejects it outright.
When the upper bound comes from a runtime value, the check moves to runtime and
panics in Debug and ReleaseSafe. Either way the check is on the slicing
operation, not deferred to the first access.
...
u8`. This has three consequences:
...
Zig leaves out a string type on purpose. A
...
The last case is an easy mistake to make. The slice is still a valid pair of
