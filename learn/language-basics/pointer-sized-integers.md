...
> usize, isize, and why they are not the same as u64.

In Zig, use `usize` for lengths, indices and byte offsets into memory, and a
fixed width such as `u64` for values whose range comes from the problem, like
a file offset or a timestamp.
...
produces code that silently narrows on wasm. So choose between them with care.
...
When this goes wrong, the code is correct on the machine you tested and wrong
on the one you shipped to. Embedded targets are
...
When the value might not fit, do the check yourself and produce an
error:
