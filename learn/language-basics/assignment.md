...
error, and so is declaring a `var` you never mutate. If you come from C, you
may not expect the second rule. Because of it, a `var` in Zig always means
*this changes*.

This helps when you read unfamiliar code. Every `const` is a value you
...
These are two separate questions. The answer to one says nothing about the
other.
...
This explains an error message you will see early. A bare `5` is not an
`i32`. It is a `comptime_int`, an arbitrary precision integer
that exists only during compilation. It coerces to whatever the context needs,
and when there is no context, there is nothing to coerce to:
...
A `var` has to live in memory at runtime, and memory cannot hold a value of
arbitrary precision. The fix is always the same: say what size you meant.
...
Zig fills the memory with `0xaa` bytes, so the mistake is easy to notice
instead of looking like a normal value.

The `0xaa` choice is deliberate. As a pointer it is wildly out of range. As a
length it is enormous. As a float it is a signalling NaN. As a signed integer
it is negative, which is easy to spot. A bug that reads uninitialised
...
before. So `undefined` is for buffers you are about to fill, and
`var x: i32 = 0` is the right thing to write when you actually meant
zero.
