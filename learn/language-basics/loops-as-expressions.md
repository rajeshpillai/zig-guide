...
Zig's `for` and `while` loops are expressions, so a search can return its
result directly, with no mutable `result` variable declared before the loop.
...
skips it. This rule makes the construct safe for searches. There are exactly two ways out of the loop, and each one produces a
value.
...
The error can be confusing, because it does not mention `else` at all.
...
If the arms really have different types, the error names both types and points at the
`break`. That check is the reason a search cannot accidentally return an index
on one path and a boolean on another.

## When to reach for it

Use the pattern when the loop answers exactly one question: find
