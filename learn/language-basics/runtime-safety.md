...
Plain `+` does not guess whether you wanted the value to wrap. Wrapping is a
reasonable thing to want in a hash function or a checksum, so Zig gives it a
different operator and makes the choice visible at the call site:
...
C never gives you this choice explicitly. `ReleaseSafe` keeps the checks and
is a reasonable default for production. `ReleaseFast` removes them for speed.
...
in `ReleaseFast`. The only difference is whether you find out. The checks do
not define what correct code is. They report code that breaks the rules. If a
program panics under `ReleaseSafe`, building it with `ReleaseFast` does not fix
the bug. The bug is still there, and the program no longer reports it.

</aside>

You can override the setting for a scope with `@setRuntimeSafety(true)`, which
is useful for keeping bounds checks in one tricky, performance-critical function
of an otherwise `ReleaseFast` build.
...
1.1 MB, against the 79 KB a snippet here usually weighs. The extra size is the
checks and the code that prints their messages. It downloads only when you
press Run on that particular snippet.
