...
Zig's tagged union, written `union(enum)`, is a sum type: the value carries a
tag naming its active field, and a `switch` on it must handle every field. A
bare `union` has no tag, and reading a field that is not the active one is
illegal behaviour.
...
"Illegal behaviour" means more than "you get garbage". In a safety
...
switch must be exhaustive. A tagged union is Zig's sum type, and it is what
you almost always want. The same construct is what makes error unions and
optionals work.
...
[logging](https://www.ziglang.in/learn/standard-library/logging/).
...
Anywhere a value is one of several shapes. A token from a lexer. A
...
interpreter on exactly this construct, so you can see it used in a real
program.
