...
In Zig, a C `int` is `c_int` and a C `long` is `c_long`, and their sizes
follow the target's C ABI. Use them at the C boundary, because `long` is 8
bytes on 64-bit Linux and 4 on Windows.
...
Zig provides `c_char`, `c_short`, `c_int`, `c_long`, `c_longlong`, the
`unsigned` counterparts of all but `c_char`, and `c_longdouble`. There is no
`c_uchar`. Their sizes are whatever the target's C ABI says.

## Why not just use `i32`?

Because a C `long` does not have one size. Code that hardcodes `i64` for it is
...
`long` causes the most trouble, because the two most common desktop platforms
disagree about it. `c_long` exists to get this right on every target Zig
supports.
...
That separation follows the general pattern here. C reuses one keyword for
"nothing" and for "something whose type I am not telling you", and code has to
work out which from context. Zig gives them different names, and the ambiguity
disappears at the point of translation rather than being carried into your
program.
