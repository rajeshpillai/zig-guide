...
In return for the extra step, a C import is now cached and
...
undefined symbol: calc_add`. This is the more confusing of the two errors, so
learn to recognise it. The line is not from memory: dropping the C file
...
## Compared with binding generators

Other languages solve the same problem with a separate tool. Python has ctypes and cffi and a generator; Rust has bindgen; Go has
...
Zig reads the header as part of the build. The binding is remade whenever the
header changes, so it cannot drift from the header it was made from. You do not
commit a generated file or regenerate one by hand.
Update the C library and a changed signature is a compile error the same day.

Remaking the binding on every build is what lets this guide's X11 chapter link against the real
...
You write this once for each function. The work is repetitive, and it is all
the wrapper has to do. In return, the rest of the program uses `try`, and no
caller has a return code it could forget to check.
