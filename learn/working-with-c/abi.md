...
To pass a struct to C from Zig, declare it `extern struct`, which uses the C
ABI's field order and padding. `packed struct` gives exact bits with no
padding, for registers and wire formats.
...
a name appears in the symbol table. The source language does not say any of
this. The platform decides it, and both sides have to already agree,
because by the time they meet there is no type information left to check.

Because nothing is left to check, mistakes here are quiet. A mismatched field offset is not a
...
Do this at any real C boundary. It turns a silent mismatch into a build
failure.

The stronger version of the same idea is to not hand-write the declaration at
all. translate-c reads the header and produces the struct, so there is nothing
...
`export` does both jobs. It gives the function an unmangled symbol name and
the C calling convention, so the `callconv(.c)` above spells out what `export`
already implies. Writing it costs nothing and says what you meant.
`@export(&add, .{ .name = "add" })` is the case where you supply the
convention yourself, because it exports a function that keeps whatever
convention it declared.
