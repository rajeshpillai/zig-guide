...
is newer than its source, so it was skipped. That is the whole
...
**The `state` array is not bookkeeping, it is the cycle check.** Each target is
...
`error.CircularDependency` rather than recursing until the stack runs out. Real
