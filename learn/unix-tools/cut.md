...
prints the whole line rather than nothing. Does the rule above explain that,
...
`dave` gives `"dave"` for field 1 and no field 2. POSIX makes `cut` pass an
undelimited line through intact whatever field you asked for, on the grounds
that dropping a line you did not parse is worse than passing it on. `-s` is
the flag that turns the exception off.
