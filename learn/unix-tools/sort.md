...
is the chapter where the streaming loop stops working. It raises a question the
earlier tools never faced.
...
right bytes. This is the ownership question from [Who Owns This
Memory](https://www.ziglang.in/learn/systems-from-scratch/who-owns-this-memory/). `sort` is the first
program where you cannot avoid it.

There are two workable answers. Copy each line into memory you allocated, or
keep the whole input in one buffer and sort a list of *pointers into it*. The
second is what the real `sort` does for anything that fits, because moving an
...
**`Zebra` sorted before `apple`.** This is not a bug. It is byte order. `Z` is 90 and `a` is
97, so every capital letter sorts before every lower-case one. This is what
`LC_ALL=C sort` does. Plain `sort` follows the locale's collation instead,
which puts `apple` first, and that is why the two disagree on the same file. Anything friendlier than byte order is a locale
...
like any other allocator. That is the correct failure for a tool with a fixed
memory budget. A C version would use a hand-written arena for the same job,
with the bookkeeping written out by hand.
...
Zig removes two problems. `qsort`'s comparator receives `const void *` and
starts with a cast. The classic bug is in that cast. The argument is a
