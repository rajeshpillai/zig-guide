...
Every tool so far moved bytes that already existed. `printf` creates new ones.
Given the number 42, something has to produce the two characters `4` and `2`.
...
quotient is zero. That gives you every digit correctly, but in reverse order,
because you get the last digit first.
...
you stopped. You never need to reverse the digits in a second pass.
...
than zero, so for zero it never runs and you get an empty string. A
hand-written integer formatter needs this special case at the top. Without it,
a count of 0 prints as nothing.

**The most negative number printed correctly.** This is the main trap in this
program. The obvious way to handle a negative is to note the sign
...
program instead of printing nonsense. That is [the main
reason](https://www.ziglang.in/learn/systems-from-scratch/when-checks-are-off/) to develop in a
safety build.

**The scanner is a state machine with two states**: copying bytes, and holding
a `%`. A format string needs nothing more. `%%` exists because once `%` is
special, you need a way to say you meant the character itself.

**An unknown specifier printed itself back.** `%q` came out as `%q`. The
program did not drop it or guess what it meant. When you cannot know what was
intended, printing the input unchanged helps the user find their typo.
...
memory you do not own. You have to work out this buffer size by hand, and it
has to be right. So real implementations write the bound next to the buffer
instead of picking a round number.
...
the types in the format string match what you passed. The language does not
check it. `%d` with a `long` is undefined behaviour, and it is a common way for
a correct-looking `printf` to corrupt a program. Modern compilers warn via
`__attribute__((format))`, but that is a warning added later, outside the
language itself.

Zig has no variadic functions on purpose. `std.fmt` takes a tuple, and the
format string is checked at compile time against it. A mismatch is a build
error instead of a problem at run time. The chapter on that is
