...
callconv(.C) void` and does not compile. A compile error is the better outcome here. An enum
parameter also means a `switch` over the signal in a shared handler is
checked. And `.USR1` cannot be confused with the integer 10 on one platform
and 30 on another.
...
is small. It does not include the calls you would use to debug why the handler
is not working.

So the handler stores to an atomic and returns. Everything real happens in the
main loop, which reads the flag between two units of work, when it has time to
handle it. It is an atomic rather than a plain `bool` because the
...
losing the notification. It is also the mechanism behind `signalfd`. Block a
signal permanently, then read it as data on a descriptor, and an interruption
becomes something a normal event loop can wait on. Most servers handle signals
this way.
...
shutdown. `SIGKILL` and `SIGSTOP` cannot be handled or blocked at all. This
guarantees that an operator can always stop a process that no longer responds.
