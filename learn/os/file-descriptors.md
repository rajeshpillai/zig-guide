...
small non-negative integer, and that integer is all you hold.
...
its methods. It is a descriptor and one bit saying whether it was opened
...
The numbering is a convention. But the kernel and every program agree on it,
so in practice it is as binding as a rule. `zig build`,
...
The descriptor number is not an implementation detail of Linux that leaked
into the API. It is the API. It is thin enough that a wasm runtime in a browser tab can implement it and a program
written against a kernel cannot tell.

## Where they come from next

The rest of this section uses descriptors in four ways: writing to the ones
you were given, passing modified copies to a child, holding one end of a pipe,
and closing one to say you are finished. Sockets in
