...
decided on and the kernel copied into your address space at startup. It
belongs only to your process. Nothing you do to it after startup reaches any
...
The count is 0 in the playground above. This is the real result, not a stub. A
wasm module in a browser tab has no parent process to inherit an environment
from, so there really are none. Reading `PATH`
there returns `null`, which is exactly what a program should be prepared for
on any platform. If you did not set a variable yourself, it may not be
there.
...
Many people expect the other direction to work, and it does not. There is no
call that lets a program change its parent's environment. A tool that wants to
alter your shell prints something for the shell to evaluate instead. The
