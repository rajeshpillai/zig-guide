...
There is no `/bin/echo` in the snippet, on purpose. A chapter that runs a
system binary fails on any machine where that binary lives somewhere else. This
site claims that CI ran every snippet today. `executablePath` gives the running
program its own path, and a first argument tells the copy which part of the
program to run.

Programs also do this in practice. A parent that re-executes itself with a flag is
...
and standard error to end of file, wait, and hand you both buffers. Use it when
the output is small and you want all of it. Do not use it when the output is
large, or when you need to react to the output as it arrives. `.stdout_limit`
caps how much it will collect. Past that cap it returns `error.StreamTooLong`,
instead of filling memory while a child keeps writing.

`std.process.spawn` returns as soon as the child exists. After that, you
control everything: the streams you asked for, when to read them, and when to
wait. [Pipes](https://www.ziglang.in/learn/os/pipes/) uses that call.

Both go through `Io`. Creating a process is an operation on the interface, not
a free function that calls the kernel without your knowledge. So a program written
...
Of the 256 codes a program can choose, only 0 means success. `&&` in a shell,
a failing step in CI, and `Term.success()` all ask that same question and all
get the same one-bit answer. Everything else you want the caller to know goes on standard error,
because a code has no room for it.

The snippet's child exits 3 and the snippet itself exits 0, on purpose. Every snippet on this site is gated on exiting 0. A failing
...
parent's `PATH`. Setting a different `PATH` in the child's environment does not
change where the child is looked up.
