...
In Zig, `std.fmt.parseInt(T, text, base)` converts a string to an integer and
returns an error when the text is not a number or does not fit in `T`. Bytes
become hex with `std.fmt.bytesToHex` and base64 with the codecs in
`std.base64`.
...
Report the two errors differently to a user.
...
Because the target type does the bounds checking, you write no separate range
validation. The check cannot disagree with the field it feeds, because the
field's type *is* the range. Parsing into `u16`
...
People often skip the sizing step, and a wrong guess does not fail loudly. On
