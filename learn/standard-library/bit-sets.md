...
`std.bit_set.Static(N)` stores a set of the integers `0..N` as bits, with no
allocator. `EnumSet`, `EnumArray` and `EnumMap` turn an enum into a set, a
table with a value for every tag, or a map with values for some tags.
...
Older code calls it `std.StaticBitSet`. The bits are stored inline. Inside,
it is a single integer when `N` is small enough, and an array of words
otherwise. The API is the same either way: `set`, `unset`, `isSet`, `count`.
When the size is only known at run time, use `std.bit_set.DynamicManaged`,
which does take an allocator.
...
light state. `EnumMap` is the partial version, for when a missing key means something.
All three are backed by the same bit-set machinery, so they are compact and
allocation-free.

This is the main reason to use `EnumArray`: it must cover every tag. Adding a
tag to the enum turns every `EnumArray` initialiser into a compile error
naming the missing case. An exhaustive `switch` gives the same guarantee, for
the same reason. A `HashMap` keyed by the enum would have compiled and
...
sixty-four thousand flags, the bit set is 8 KB and sits in L1 on most
machines. The array of `bool` is 64 KB and does not. "Is any of these set"
goes from a scan to a handful of `or`s.

The other thing it replaces is the hand-rolled `flags: u32` with named
constants and manual `&` and `|`. That works, and it is what C does. But the
bit positions live in comments, nothing stops you testing the wrong one, and
...
runs of zeroes a word at a time instead of testing every index. For a sparse
set over a large range, the loop then takes time in proportion to the number
of members, not to the size of the whole range.
...
Because `std.bit_set.Static(N)` stores its bits inline, the type is copyable
and has no pointers inside. It can live in a struct field or be returned by
value. That makes it the default choice.

`DynamicManaged` is for when `N` is decided at run time, and it takes an
allocator and needs a `deinit`. Before using it, ask whether you really have
no upper bound. A static set sized to a generous maximum is usually both
