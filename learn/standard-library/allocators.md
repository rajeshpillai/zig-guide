...
The arena fits more programs than you might expect. If your program does a
unit of work and then throws everything away, an arena turns hundreds of
`free` calls into a single `deinit`. It also removes a whole category of leak.
...
returns. Zig code writes the `free` on the line directly *after* the `alloc`,
so you see the pair together.

Note the test above uses `std.testing.allocator`: if a test leaks, the test
fails. Leak checking is the default in tests. You do not have to remember to
run a separate tool.
...
The new name says "safe" instead of "debug" because the checks are useful
outside a development build. It panics on a double free, on a free through the
wrong allocator, and on most writes after free. You pay for it in
