...
In Zig, `std.mem.sort` sorts a slice in place, stably, using a less-than
function you pass, and `std.mem.sortUnstable` is the version that gives up
...
The context parameter is what saves you from reaching for a global.

The pattern has more uses than it first seems. Sorting an array of
...
Multi-key ordering is built out of that. Sort by the least significant key
first, then by the most significant, and stability preserves the earlier work
inside each group. Sort by name, then by department, and each department's
entries are still in name order. Do the same with `sortUnstable` and the
second sort scrambles the first.

When elements are fully interchangeable, such as plain integers, you cannot
see the difference stability makes. Then `sortUnstable` is the better choice:
it does less work and uses no extra memory.
...
half to keep. `std.sort.isSorted` checks the invariant, and it is a cheap
assertion to put in a test.

Two related functions help when you do not need the whole thing ordered.
