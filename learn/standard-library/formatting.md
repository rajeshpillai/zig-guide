...
A format string is a string known at compile time, with one `{}` placeholder
per argument.
`{d}` prints a number, `{s}` a string, and `{any}` anything else. The
compiler checks the placeholders against the arguments, so a mismatch is a
compile error rather than wrong output at runtime.
...
Right is the default for every type, strings included, and this often causes
mistakes: `{s:8}` puts the padding before the text, not after. Lining up a column
