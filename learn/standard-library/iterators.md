...
Zig's standard library iterators, such as `std.mem.splitScalar`,
`std.mem.tokenizeAny` and `std.mem.window`, are plain structs with a `next`
method, and they yield slices of the original bytes without allocating.
...
To write your own, make a struct with state and a `next` method.
...
stay valid as long as the original does. The usual downside applies: if the
source is freed, every piece you kept is dangling.

`peek()` looks at the next value without consuming it. `rest()` gives you
everything not yet returned. Reach for it when a header is parsed field by
field and the remainder is a body. `reset()` starts over.

## Windows

`std.mem.window(T, s, size, advance)` yields views into the original bytes
with no copying, the shape n-gram and block processing want. Overlap is up
...
Chains were left out because Zig has no closures. A `map` would have to take
a function pointer plus a context struct. That is three lines of setup for
what one line of loop body says directly. Chains read well in languages where
a lambda captures its environment automatically.
