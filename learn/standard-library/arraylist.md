...
`std.ArrayList` is Zig's growable array, and on master it holds no allocator:
create it with `.empty` and pass the allocator to every method that might
allocate, as in `list.append(gpa, x)`.
...
The second reason for the change shows up at scale. A program holding ten
...
An `items` slice, a `capacity`, and a safety field covered further down that
costs nothing outside a safety build. Three words, then: a pointer, a length,
and how many elements the buffer could hold before it has to grow. Appending
past the capacity allocates a larger buffer, copies, and frees the old one.
That is why append returns an error, and why growth has an amortised cost.
...
Left alone, nothing catches this. The old slice is still a valid pointer and
a valid length; it just points into memory that has been freed. It reads
plausible data or crashes, depending on what the allocator did next. The same
applies to a pointer to an element: `&list.items[0]` is invalid after any
append that grows.
...
## Locking the buffer in place

`lockPointers` and `unlockPointers` mark a region where the buffer is not
allowed to move. Inside it, an operation that would reallocate or free panics
rather than invalidating a pointer you are still holding:
...
It fires on the invalidation, not on the call. An `append` that fits in the
existing capacity moves nothing and passes; the same `append` panics on the
iteration that has to grow the buffer. Methods that always invalidate assert
directly, `insert`, `swapRemove` and `toOwnedSlice` among them.

The field behind it is a `std.debug.SafetyLock`, which checks nothing outside
a safety build. Debug and ReleaseSafe have it. ReleaseFast and ReleaseSmall do
not, and the wasm behind the Run button on this page is ReleaseSmall.
...
This is not only about speed. A loop of `try list.append(...)` has an
...
[queue](https://www.ziglang.in/learn/concurrency/queues/) is not. When lookup is by key rather
