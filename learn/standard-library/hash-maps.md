...
In Zig, a hash map is `std.AutoHashMapUnmanaged(K, V)`, or
`std.StringHashMapUnmanaged(V)` when the keys are strings, and
`map.getOrPut(gpa, key)` inserts or updates an entry with a single lookup.
...
The refusal is a compile error, not a runtime surprise. It is a good example
of what comptime reflection can do. The map inspects your key
...
The third row is easy to miss. A plain hash map's iteration order is
...
test that iterates a plain hash map will eventually fail for no visible
reason.
...
There is a whole family of these. `put` overwrites, `putNoClobber` asserts
...
The duplicating version is short. Keep it as a template:
...
`deinit`. Forgetting that is a common leak in Zig code that uses maps,
