...
Zig tests are `test` blocks run by `zig test`, and `std.testing` supplies the
assertions, such as `expectEqual` and `expectError`, plus
`std.testing.allocator`, which fails a test that leaks memory.
...
when the test returns. So a missing `free` fails the test instead of becoming
...
Use it in every test. Memory bugs are often hard to reproduce. They may not
show up in the test that caused them, and can become a production incident
weeks later. A test that uses this allocator is also a leak test, with no
extra code.
...
that every partial failure still cleans up. That test is too tedious to write
by hand, and it is the only way to find out whether your chain of `errdefer`
statements is right.

## How tests are found and run

A `test` block is a top-level declaration, and `zig test` compiles the file
into a runner that executes every one it can see. "Can see" follows a rule.
Tests in a file that nothing imports are never compiled, so a
...
full of them for that reason, and your code can do the same. A reader looking
at an unfamiliar function will read the test before the doc comment, because
the test cannot be out of date.

This whole site works the same way. Every snippet here is compiled and
...
passed, so a permanently skipped test stays visible in the output.

For code that should not compile, there is no assertion to write. The compile
error already fails the build. When the type rejects a misuse, you do not
write that misuse in a test. You rely on the type.
