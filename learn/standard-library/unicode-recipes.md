...
To truncate a UTF-8 string in Zig without cutting a character in half, walk
`nextCodepointSlice` and stop before the slice that would pass the limit.
Conversion to and from UTF-16 is `std.unicode.utf8ToUtf16LeAlloc` and
`utf16LeToUtf8Alloc`.
...
Handing it a continuation byte returns `error.Utf8InvalidStartByte`, the check
you want when an index came from arithmetic rather than an iterator.
...
character. Nothing fails at the cut. The failure surfaces wherever those bytes
are decoded, possibly in another program.
...
You can remember this rule instead of looking it up. A UTF-8 character is at
most four bytes, and the format was designed so this walk is always short and never ambiguous.
...
bad sequences instead of returning an error. A log line almost always wants
this. When you are debugging bad input, you still need to see it printed.
...
the byte counts. Because of that difference, the conversion is explicit and
never automatic.

## What std does not do

There is no grapheme-cluster support, no display width, no case folding, no
normalization, and no case mapping past ASCII. A hand-written loop cannot fill
this gap. `é` can be one codepoint or two, and the two forms compare unequal.
Deciding they are the same string needs the Unicode tables. Those
