...
In Zig, a custom `format` method takes only a `*std.Io.Writer`, and `print`
calls it only when the format string asks with `{f}`. Width, fill and
alignment go after a colon, as in `{d:0>4}`, and are the formatter's job.
...
test "format strings are checked at compile time" {
    ...
    // not support, is a compile error, not a runtime error.
...
The reason for the change explains the rest of this page. Under the old design, every type's `format` had to parse the format
...
This is the other half of the same change, and it often breaks code being
ported. Under the old scheme `{}` found your method, so a type with a `format`
printed nicely everywhere without you asking. Now `{}` prints the struct the
way the compiler sees it, and `{f}` asks for yours. The benefit is that the
call site now shows whether you get the type's own rendering or the generic
dump. Before, you could not tell.
...
error, not a runtime one. This check is why the format string has to be a
comptime value.
...
implements. So the same `print` call works against all of them, and a
function taking a `*std.Io.Writer` can be tested by handing it a buffer.
