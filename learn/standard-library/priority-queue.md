...
`std.PriorityQueue(T, Context, compareFn)` is Zig's binary heap. `push` and
`pop` are O(log n), and `pop` returns the element the comparator ranks first,
...
"first in, first out", that is a [queue](https://www.ziglang.in/learn/concurrency/queues/) and
