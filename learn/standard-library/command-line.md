...
A Zig program reads its command-line arguments from `init.minimal.args`, the
`std.process.Args` inside the `std.process.Init` that `main` receives, and its
environment from `init.environ_map`. The standard library has no argument
parser, so the flag loop is yours to write.
...
the snippet is usually all you need: match a flag, and pull the next token
when it takes a value. The first argument is always the program path, so skip
...
Your own loop has to follow these conventions, because users expect them:
...
None of this is hard, and all of it is easy to leave out. Decide up front
which of these you support, so you do not find the gap from a bug report.
