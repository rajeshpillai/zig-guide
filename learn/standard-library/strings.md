...
In Zig, a string is a `[]const u8`, and searching, trimming, splitting and
joining are free functions in `std.mem`, such as `std.mem.find`,
`std.mem.trim` and `std.mem.splitScalar`.
...
A string is `[]const u8`: a slice of bytes. That means you cannot compare two
strings with `==`. The compiler rejects `==` on slices. Content comparison is `std.mem.eql(u8, a, b)`. To put two strings in order,
`std.mem.order(u8, a, b)` returns `.lt`, `.eq` or `.gt`. Everything else you would
expect from a string class is a free function in `std.mem` that works on any
slice type. Because they work on any slice, each call names the element type
first.
...
Nothing fails, so a guide could teach the old spelling for another year and
never find out.
...
`trim`, `split`, and `tokenize` return views into the original bytes. They do
not copy, so the result is only valid as long as the original. `join`, `replaceOwned`,
