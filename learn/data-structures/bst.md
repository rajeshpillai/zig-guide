...
walk visits the values in sorted order with no extra work.

Both savings hold only while the tree stays roughly balanced. Nothing in the
insert code makes it stay balanced.
...
Read `*?*Node` slowly: a pointer, to an optional, to a node. Each
layer is doing something. The optional is the empty slot; the outer pointer is
what lets you write into it.
...
Recursion here is bounded by the tree's height, the number the next section is
about. On a degenerate tree of a million nodes it would overflow the stack.
Production tree code often keeps an explicit stack for that reason.
...
An unbalanced binary search tree fails exactly there, and sorted input is not
an unusual case. It is what you get from a database export, a
...
- **`std.Treap`**, already in `std`, which keeps itself balanced
  probabilistically rather than by rotation.

Which leaves the case where you really need ordered iteration with
interleaved inserts. For that the tree has to repair its own shape. The next
chapter makes it do so.
