...
types, so a node from one list cannot be handed to the other. This follows from the structure. Nobody had to write it down as a rule.
...
Be careful with one case: it compares slices by pointer and length,
...
One difference to copy: on master `ArrayList` does not store the
