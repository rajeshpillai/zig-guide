...
> Open addressing with linear probing, growth at 75%, and the tombstone, which looks like a hack but is what makes delete work.
...
exist further along the chain. Only when it reaches an empty slot
...
work per delete. Robin Hood hashing usually pairs with it. So tombstones are a design choice. The choice is between cheaper deletes and
shorter probes.
...
[Hash maps](https://www.ziglang.in/learn/standard-library/hash-maps/) covers the API. The main structural difference is that `std` stores a separate array of one-byte
