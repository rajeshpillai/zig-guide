...
The result on the input that broke the plain tree:
...
A factor of 2 means one side is a full level deeper than the other, and that
is the moment to rotate. The bound keeps the height within about 1.44 log₂(n).
A red-black tree allows a looser one, so an AVL tree does more rotations on
insert and gives slightly faster lookups in exchange.
...
preserves the ordering, so it can be applied anywhere at any time without
checking whether it breaks the search property. Only `B` changes parent.
...
Four cases, and they are mirror images in pairs.
...
that is subtly wrong produces a tree that is well balanced and has lost a
subtree.

When a structure exists to maintain an invariant, the invariant check belongs in the code and not only in the prose.

## Deletion, and why it is not here
...
Most people switch to a red-black tree at that point, since its looser rule
makes deletion cheaper. The other option is a treap, which avoids the case
analysis entirely by giving each node a random priority and keeping a heap on
it.
