...
outside in, and getting it backwards is a common early mistake with
linked structures.
...
anyone else, so the bug often ships.
...
A container that hands out memory is exactly where a leak comes from, so let the allocator check rather than reasoning about it. [Catching
...
list first, and building a list of n items is O(n²). Many hand-written lists leave it out.
