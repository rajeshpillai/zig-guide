...
On Zig master today that line itself does not compile, before any of raylib's C
is built.
...
master. The breakage comes from that difference, so it is better to know about
it before you add the dependency.
...
`vendor/raylib` stays a plain directory of C
...
That is six C files. `rglfw.c` is raylib's bundled GLFW, the windowing layer.
...
No ALSA package is needed. raylib's audio backend loads libasound at runtime rather than
...
approach. When raylib changes its configuration, this function has to follow.
The macro list is a dozen lines, and a mismatch fails at compile time rather
than quietly producing a different library.
