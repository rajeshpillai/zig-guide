...
If the view stretched to fill the window, a wide window would show blocks
earlier, and a player could gain an advantage by resizing.
...
55 percent, so the coin never disappears. An earlier version swept the full
range, so the coin turned edge on and vanished for a few frames. Players do not
go for a coin they cannot see.
...
lines. It is most of what makes the movement look deliberate.

## Colours are plain data

<GameSource file="src/render/palette.zig" decl="Color" />

That file has no raylib types. `draw.zig` converts at the edge:
...
The particle system turns that into ten amber dots flying outwards.
...
different element type and a capacity of 256.

A full pool drops new particles. It does not stall a frame to make room,
because a stalled frame is visible and a missing spark is not.
