...
after every press, and that gap is what gives the game its feel.
...
two lanes costing exactly twice one. Constant speed looks less smooth, but it
keeps the spacing rule correct.

Because `x` is continuous, the player really occupies the space between
lanes. Clipping a block while sliding counts as a crash.
...
fraction of a point a hundred and twenty times a second would drift.
