...
Getting that wrong does not show up as a bug. Every row still leaves a lane
open and the game still runs, but nobody can survive the course. The only sign
is that people stop playing.
...
All of that is still reasoning about four constants. That kind of reasoning is
easy to get wrong, so a program plays the game instead.
...
centre is 41 units behind you still overlaps you when each body is 52 units
tall.
...
reasoning that joined them together. Only a full run of the game tests that
reasoning.
...
Everybody loses eventually. That part took the most work to get right.
...
player who chases coins follows the safe path without being told. The rule is
four statements in the spawner. The game has no other tutorial.
