...
Recording or licensing five short sounds would have been faster than writing a
synthesiser. It would also have added a folder of binary files that nobody can
diff, a licence question for each one, and an audio editor in every change. Generating them means a sound changes when a number changes.
...
Then the mix. The exact frequencies are a matter of taste and can change. How
the sounds relate to each other must not. The crash is the low long loud one. The lane tick fires on every input
...
The sound in this game is decoration, so the game must still start when a mixer
will not open. The real binary is run with a deliberately broken
ALSA configuration as part of checking that.

Sound reads the same
