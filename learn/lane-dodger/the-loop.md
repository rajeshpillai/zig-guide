...
ends of that jump rather than through it. If the game stutters, the player can
...
Drawing between states is why `Entity` carries a `y_prev` that no rule ever reads. It is not
...
calls a function once per animation frame and never returns into your code. A
`while` loop cannot run there, and its local variables would have nowhere to
live.
...
Make the split even for a game that will never ship to a browser. A frame you
can call once can also be called six hundred times, with a bot at the controls
and no window open. That is how
