...
the highest speed there are about three rows on screen, with at most three
entities each, so nine or so at once. A test asserts the peak stays under
capacity and above six, so a change that quietly stops spawning shows up.

## An index is not enough

The obvious way to refer to an entity is its index. That leads to a bug that is
...
believes it is a coin. The program does not crash. The score is wrong about
once an hour, usually when nobody is watching.
...
counter answers both "which occupant" and "is the slot in use". Generation zero is
...
test could pass because the pool handed out a fresh slot, and that case does not
test the bug.

## One pool, two users

The pool is generic, so other code can use it. The particle system uses the same
pool with a capacity of 256 and a different element type, even though particles
belong to the renderer and not the simulation. Writing it once gave the renderer
fixed-capacity storage with no allocator, at no extra cost.
...
Sound was added after the game was already playable. It did not change any rule
of the game, because it reads the same events the particles read.
...
buffer and one loop are enough here. The test of the design is whether adding a
reaction means changing the code that caused it. Here it does not. The same
property lets
