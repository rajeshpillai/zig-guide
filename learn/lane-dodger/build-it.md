...
The fetch script below records which raylib commit the game uses. The copy it
downloads is 13 MB, so it stays out of git.
...
raylib is *not* in `.dependencies`. That is deliberate, and
...
You have not written any drawing code, input code or `main.zig` yet, and no
window or audio device is needed. The game is still complete enough to be played
and checked. The design is arranged so that this checkpoint exists.

If this passes, the rules are right. The remaining files draw the game, read
input and play sound.
