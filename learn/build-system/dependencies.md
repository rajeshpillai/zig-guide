...
dependency set on any machine. The manifest records what you
depend on. The build script wires it in. Neither file can be edited into a state where the other silently
...
`build.zig.zon`, the small manifest. You do not commit a `vendor/` directory
or run a separate install step like `npm install`.
