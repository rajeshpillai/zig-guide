...
For anything up to a few hundred lines, `zig run main.zig` is all you need,
and there is no build step to remember. Arguments for the
program go after `--`:
...
You can work on a single-file tool this way for a long time.
...
`build-obj` is an easy way to bring Zig into an existing codebase. Compile one module
...
a lot here, because the artifact is downloaded before it
runs.

## When to graduate to `build.zig`

Once you have more than one artifact, dependencies, generated files, or a named
test step, move to a build script. `zig build --help` then lists the
