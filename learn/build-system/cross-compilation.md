...
You do not install a cross toolchain or assemble a sysroot. Zig ships libc sources
...
that has no Windows SDK, MinGW or Wine installed. Swap the triple for
...
Pay attention to the ABI field. `musl` produces a fully static binary. `gnu`
links against glibc and inherits its version compatibility rules. So the two
`x86_64-linux` lines above are two different builds.
...
If you have cross-compiled C before, you will notice how many steps are
missing here.
...
find both. Assembling one takes a day, and keeping several up to date is ongoing
work. Because of that cost, "we only build on Linux" is a common answer, and CI matrices spend most of their time installing compilers.
...
Cross-compiling gives you a binary. You still have to test it. It also does
not remove the platform differences the code has to handle: path separators,
line endings, the fact that `usize` is 4 bytes on wasm and 8 on your laptop.
A successful build proves the code compiles for that target. That is most of
the value, but compiling is not the same as working.

Linking against a system library the target has and your machine does not is
the other limit. That needs the real headers, and at that point cross-compiling needs extra
setup.

## Compiling C too

`zig cc` is a drop-in C compiler with the same cross-compilation support, which
...
substitution. Its source does not have to change. `zig c++` does the same
for C++.
