...
That is what makes this page possible at all. The loop reads through a
`std.Io.Reader`, and here that reader is built over a string rather than over
a terminal. Move where the reader comes from and the rest of the loop is
unchanged. It is also how you test a REPL, which is otherwise one of the
