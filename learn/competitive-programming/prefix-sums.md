...
> One subtraction instead of a loop, the spare cell that removes the special case at index 0, and the same trick run backwards for range updates.

A range-sum query asks for the total of one part of an array.

Here we need to answer many of them.
...
*Runnable: compiled to WebAssembly and executed by CI against Zig master. (`23-competitive.prefix-sums`)*
...
We define `pre[i]` as the sum of the first `i` values.
...
`pre[3]` is 8 because it holds the three values before index 3, `3 + 1 + 4`.

`pre[0]` represents the boundary before the first value.

`pre[8]` represents the boundary after the last value.

Without the extra cell at index 0, a range starting at index 0 would need a special case.
...
The cost does not depend on the width of the range.
...
The empty range `[4,4)` also works: `pre[4] - pre[4] = 0`.

No special case is required.

The full array is `pre[8] - pre[0]`, which gives 31.

Using half-open ranges also matches normal Zig slicing.

If an inclusive range is used instead, the formula becomes `pre[hi + 1] - pre[lo]`.
...
The benefit grows when there are many values and many queries.

Prefix sums also have a limitation.
...
[Sliding Window](https://www.ziglang.in/learn/competitive-programming/sliding-window/) covers that pattern.

## Difference arrays

<SnippetSource name="23-competitive.prefix-sums" decl="applyRange" />

Some problems have many range updates instead of many range queries.
...
Here are the three updates above:
...
To add 5 over `[0,3)`, we write `+5` at index 0.

Then we write `-5` at index 3.
...
For wider ranges, the gap between the two grows.
...
The next difference is `1 - 3 = -2`.

The next is `4 - 1 = 3`, and so on.

If we take a prefix sum of these differences, we rebuild the original values.

So the two techniques are closely related.

A prefix array costs O(n) to build, and then each range-sum query costs O(1).

A difference array records each range update in O(1), and costs O(n) to turn back into final values.

If you have many queries and few updates, use prefix sums.

If you have many range updates and only need the final array afterwards, use a difference array.
