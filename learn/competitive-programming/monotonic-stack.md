...
> A stack of the indices still waiting for an answer, and the two counters that show the inner loop keeps the total linear.

For every value in an array, we want the first larger value to its right.
...
Instead of asking "What is the answer for this index?", we ask which earlier indices the current value can answer.
...
For an increasing array such as `1 2 3 4 5`, each value except the last finds its answer immediately.

So each index needs only one comparison.

For a decreasing array such as `9 8 7 6 5`, no value ever finds a larger value to its right.
...
The total is `n(n - 1) / 2` comparisons.
...
Let the current value be `items[i]`.

Look at the index on top of the stack.

If `items[i] > items[stack_top]`, the current value is the next greater value for that stored index.
...
For example, if the stack contains the values `9 7 5 3`, a new value of 6 can answer 3 and 5.

It cannot answer 7.

So the popping stops there.

The stack keeps this order without any extra work.
...
Each stack entry is written as `index:value`, from bottom to top.
...
At the end, two indices remain: `5:9` and `10:7`.
...
At first this can look like O(n²), but it is O(n).

To see why, count how many times an index can enter and leave the stack.
...
So the total work over the whole array stays linear.
...
The same idea solves another common problem: for each value, find the nearest smaller value to its left.
