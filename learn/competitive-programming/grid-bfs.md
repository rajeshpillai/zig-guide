...
The queue itself knows nothing about shortest paths.

The shortest paths come from the order in which BFS visits cells.
...
Take a cell in row 0.

For that cell, the expression `p.r - 1` is invalid.
...
For example, moving up from `(0, 0)` produces `(-1, 0)`.

Both coordinates are valid `isize` values.
...
For example, test `if (p.r > 0)` before calculating `p.r - 1`.

Both approaches are valid.

Do not subtract 1 from a zero `usize` and then try to check the result afterwards.
...
Here is the search partway through:
...
A shorter route cannot appear later.
...
So BFS does not need to keep comparing possible distances for the same cell.
...
A common BFS bug is marking cells too late.
...
The correct rule is to mark a node when you enqueue it, not when you dequeue it.
...
For this grid, `9 * 13 = 117`, so 117 slots are enough.

The queue can simply allocate space for all cells once.

It never needs to grow or wrap around.
...
The overflow is another practical reason to mark on push.

A circular queue would reuse old slots and avoid the fixed-array overflow, but the duplicate pushes would still happen.
...
Because of that, distance levels line up with queue order.
