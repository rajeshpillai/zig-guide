...
> One addition and one subtraction instead of a fresh sum, the window that grows and shrinks, and the count that shows the inner loop keeps the total linear.

We have ten numbers and two questions about them.
...
The first four windows look like this:
...
The difference is small for ten values.

It grows as the array grows.
...
The `while` loop inside the `for` loop is still O(n), for the reason explained in [The Monotonic Stack](https://www.ziglang.in/learn/competitive-programming/monotonic-stack/).

Each value enters the window once and leaves it at most once.

Here `lo` never moves backwards, so the two edges together move at most about `2n` times.
...
The variable-size window above depends on one property.
...
These two facts give us a safe rule for deciding when to grow and when to shrink.
