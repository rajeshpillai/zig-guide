...
A window moves across an array one step at a time.
...
Here are both methods on the example input:
...
Two rules maintain the deque.

When a new value arrives, the back removes indices whose values are smaller.

When the window moves, the front removes indices that are now outside it.
...
The deque now contains only `2:8`.
...
The maximum is always available directly from the front, as `items[d.front().?]`.
...
So if two equal maximum values exist, the older one may expire while the newer one remains available.

## The back removes by value

The back rule answers one question: can an old value ever become the maximum again after this new value arrives?

Take an existing deque entry whose value is smaller than the new value.
...
The front rule handles a different problem.
...
This rule compares only indices.

Values play no part in it.
...
Together, the two rules keep the maximum of the current window at the front.
...
A normal queue of the last `k` indices knows which indices expire.

But it keeps values that can no longer become the maximum.

The monotonic deque removes both kinds of index.

## Why the algorithm is O(n)

The two `while` loops inside the `for` loop are still linear, for the same reason as in [the monotonic stack](https://www.ziglang.in/learn/competitive-programming/monotonic-stack/).

Each index is pushed once and removed at most once, so there are at most about `2n` deque operations.
...
[Sliding Window](https://www.ziglang.in/learn/competitive-programming/sliding-window/) uses the same kind of amortised argument.
...
The cost of the first two strategies depends heavily on the shape of the input.

The total work of the deque stays O(n) for every input.

## Reusing the same pattern

If we reverse the back comparison, the same structure gives the minimum value in every window.

For the maximum, remove smaller values from the back.

For the minimum, remove larger values from the back.
