...
The input is a sorted array and a target sum.
...
The current range is `items[lo..hi + 1]`.

Everything outside this range has already been ruled out.

At each step, we calculate `items[lo] + items[hi]`.
...
For `racecar` the walk looks like this:
...
The condition `while (lo < hi)` stops before the middle value.
...
The height is `@min(heights[lo], heights[hi])`.

So the area is `@min(heights[lo], heights[hi]) * (hi - lo)`.
...
That need decides which pointer should move.
...
The program prints the first four steps:
...
Moving the taller wall may work for some inputs, but it can remove a pair that would have produced the maximum area.
