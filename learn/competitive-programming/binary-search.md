...
Both versions compile, so we need to know exactly what each variable means.
...
The range is half-open, `items[lo..hi]`.

This means `lo` is included, but `hi` is not.

At every step, we keep one rule: every index that could still be the answer must remain inside the search window.

If `items[mid] < key`, then `mid` cannot be the answer.
...
`mid` may still be the answer, so we keep it inside the search window with `hi = mid`.

So one branch moves past `mid`, and the other keeps `mid` in the window.
...
The midpoint is calculated as `lo + (hi - lo) / 2`.

You may also see `(lo + hi) / 2`.
...
A common mistake is writing `lo = mid` instead of `lo = mid + 1`.
...
When `hi - lo` becomes 1, integer division gives `mid == lo`.

Now `lo = mid` does not change anything.
...
A simple rule helps catch this bug: the search window must become smaller after every iteration.
...
The difference is in the comparison.
...
The final line shows how `binarySearch` and `lowerBound` differ.

For key `5`, `binarySearch` returns null:

```
binarySearch(5) = null
```

The value `5` is not present.

But `lowerBound` returns 4:

```
lowerBound(5) = 4
```

Index 4 is where `5` would be inserted.
...
So the same parsing function can work with different input sources.
