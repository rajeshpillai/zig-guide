...
returns response bytes. The in-memory store needs no server, no socket
and no database. That is why this chapter runs in your browser.
...
route shadowing arrives silently. Someone adds `/stats` next to a
