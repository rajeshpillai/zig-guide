...
A PostgreSQL client in Zig speaks the wire protocol directly: `PgClient`
sends the startup message, answers the server's password request,
returns rows through an iterator, and turns an `ErrorResponse` into a
Zig error without losing the connection. It reads from a `Reader`, so
its tests run against scripted bytes instead of a database.

## The problem

The [cookbook recipe](https://www.ziglang.in/learn/how-to/databases/postgres-wire/) showed the
wire protocol is just tagged, length-prefixed messages. A recipe is not
a client. The service in this project needs a client it can call
without thinking about bytes.

Keeping the connection after an error is important. A shortener will
hit constraint violations in normal operation. A client that treats
every error as fatal would reconnect all the time.
...
the difference, because it parses a `Reader` and does not know where the
bytes came from. This guide uses the same seam in other chapters. It
lets a driver have regression tests without a database in CI.
...
It fits into the same `switch` as one more case, and the
...
buffer, valid until the next message is read. Real drivers work the
same way, for the same reason: the alternative is an allocation per
...
the comment on `drain` in that file states this rule in one line.
...
violation), then a `ReadyForQuery`. For the server, only the transaction
failed, and the connection is still fine.
