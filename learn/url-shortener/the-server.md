...
`sqlQuote` doubles every `'`. Understand what that buys and what it
does not. It stops a value from terminating its own string literal,
which is the classic injection. It is also the method the simple query protocol forces,
