...
and anyone who follows the short one lands on the long one. It is a
common first service. It is small enough to finish, and it still needs
everything a bigger service needs: a database, a
...
string. Following it gives the redirect, which is the service's main job:
...
The routes chapter's expected output contains these same exchanges,
and CI replays them on every build.
...
This project turns that recipe into something a program can use: a
client that connects, authenticates, sends SQL, iterates rows, and
survives errors. That is what a driver is. Writing one small enough to
read means no part of the path between your code and the database stays
hidden.

## Decision two: the seams

Almost nothing in this project needs a network to run. This follows the
same rule as the rest of the guide: protocol
...
This small project skips some things. Each gap below could be filled
behind one of the seams in this design.
