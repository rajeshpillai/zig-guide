...
A SIMD dot product in Zig multiplies `@Vector(4, i32)` chunks with plain `*`
and `+=`, collapses the lanes with `@reduce(.Add, acc)`, and finishes the
leftover elements with a scalar loop.

## The problem

You have two arrays of numbers and want their dot product, and the scalar
loop shows up in your profile. Modern CPUs (and wasm) have SIMD registers that
...
The two parts that are easy to get wrong are the ends: turning slice data into vector
...
`xs[i..][0..Lanes]` is a common idiom. The first slice moves the start. The
second has a comptime-known length, so its type is a pointer to an array,
`*const [4]i32`. Dereferencing with `.*` yields the array by value, and
arrays coerce to vectors of the same length and element type. This needs no
`@memcpy` and no unsafe cast, and the bounds are checked in safe builds.
...
checks itself against the scalar version to show both give the same result.
...
whatever the CPU offers. Measure before assuming it is faster. For small
arrays the setup and reduce can cost more than the loop saves.
...
- **Floats:** the same code works with `f32` lanes. Float addition is not
  associative, so vector and scalar sums can differ in the last bits.
