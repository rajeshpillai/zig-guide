...
Zig prints any struct without a per-type function by looping over `@typeInfo`
with `inline for` at compile time. On current master the fields come as the
parallel arrays `field_names` and `field_types`, not the old `fields` slice.

## The problem

You want to dump any struct for debugging or [logging](https://www.ziglang.in/learn/standard-library/logging/): field names, values,
strings quoted, without writing a print function per type and without runtime
reflection machinery. In most languages this needs a reflection API and some
runtime overhead.
...
If you learned this API before, note the change: `@typeInfo(T).@"struct"` used
...
compiles. The approach is the same. Only the shape of the type info changed.

## Why the branch costs nothing

`if (comptime Field == []const u8)` is decided while compiling, per field.
For `host` the compiler emits only the quoted-string print. For `port` it
emits only the `{any}` print. The other branch is not compiled at all, which also means
...
- **Enums and unions:** `{t}` prints tag names directly. Combine it with this
