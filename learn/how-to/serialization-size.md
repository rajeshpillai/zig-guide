...
The same 100,000 records written with Zig's `std.json.Stringify.value` take
4.62 times the bytes of a fixed 24-byte binary record written with
`std.mem.writeInt`. JSON repeats every field name and spells numbers out as
decimal text.
...
prints the exact byte counts, so you can compare measured numbers.

The data is seeded, so the sizes are reproducible. You get the same numbers
every run, and that is also why this page can assert them.
...
serializes as `"move"` or `"attack"`. JSON pays this cost to be
self-describing.
...
bytes are the same on every host. Without that, the output would be a memory
dump and not a format. [The binary round-trip
recipe](https://www.ziglang.in/learn/how-to/binary-roundtrip/) covers this in detail.
