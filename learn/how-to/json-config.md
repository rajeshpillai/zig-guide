...
Zig's `std.json.parseFromSlice` loads a JSON config into a struct, and any
field with a default value keeps that default when the file leaves it out.
Checks such as a port range go in a separate `validate` function, because the
parser only checks types and field names.
...
- **Empty input:** `{}` yields the all-defaults config. If that case errors,
  some field is missing its default.
...
- **Typo'd field:** `"prot"` fails with `error.UnknownField` because
  rejecting unknown fields is the parser's default. Keep that default. It
  catches a field the user meant to set but misspelled. Opt out with
  `.ignore_unknown_fields = true` only when consuming payloads you don't
  fully model.
- **Not JSON:** the parser reports a syntax error. Nothing was allocated, so
...
- **Nested config:** struct fields can be structs, arrays, optionals, and
  enums. The parser maps JSON onto all of them. Enum fields also give you
  "one of these strings" validation with no extra code.
