...
Zig's math functions live in `std.math`: `std.math.sqrt`, `std.math.sin`
(which takes radians), `std.math.pow(T, base, exp)` and constants such as
`std.math.pi`. Floats are compared with `std.math.approxEqAbs`, not `==`.

## The problem

You need a formula: a sine, a square root, degrees turned into radians. The
functions are all in `std.math`. Two details cause most first mistakes: trig
works in radians, and `pow` wants its type as the first argument. This is the
short reference.
...
`1` because of the `{d:.4}` format, and at `f32` the result really is exactly
one. Do not read a rule into that. For most whole-degree angles the `f32` sine
differs in the last bit from the correctly rounded value, which is why the
comparison below uses a tolerance.
...
you should test floats for equality. Plain `==` on floats is unreliable.
Because of rounding, values that are mathematically equal often differ in the
last bits, and values that look different can compare equal. Whether
`0.1 + 0.2 == 0.3` holds depends on the type and on when the sum is computed.
At `f32` it happens to be true. At `f64` computed at runtime it is false.
Computed at compile time, it may round back to true. Because the answer
changes like this, never rely on `==` for floats. The
