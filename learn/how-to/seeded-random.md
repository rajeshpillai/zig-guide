...
tokens, and anything an attacker must not guess want operating system
entropy. On master that arrives through `std.Random.IoSource`, which fills
...
so the capability is passed in like every other effect. The dividing line: if
you would ever want to replay it, use a PRNG; if replaying it would be a
