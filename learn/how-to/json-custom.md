...
To control how Zig writes a struct as JSON, give the struct a `jsonStringify`
method that drives the write stream with `beginObject`, `objectField`, `write`
and `endObject`. Renaming a field or adding a computed value then takes no
string building.
...
When the type declares `pub fn jsonStringify`, `std.json.Stringify` calls it
instead of walking the fields.
...
escaping. Prefer `write` unless you specifically need the raw form. `write`
