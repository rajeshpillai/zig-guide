...
Zig calls SQLite's C API directly: a translate-c build step turns `sqlite3.h`
into a module imported as `@import("c")`, and the program links `sqlite3`.
`@cImport` was removed, so older examples that use it no longer compile.

## The problem

You want a real database from Zig, and SQLite is the obvious first one. It is
a C library with no server, so all you need is a header and a linked
...
the module to the snippet. As a result, C imports are cached and configured
like any other dependency, instead of being re-run on every compile.
...
concatenation. Placeholders keep user data out of the SQL grammar, so a name
containing a quote is data, not syntax. Binding with `SQLITE_STATIC`
