...
`std.testing.allocator` is a `std.heap.SafeAllocator`, which keeps its own
record of every live allocation regardless of how the code under test is
compiled, so CI catches leaks without shipping a debug build.
