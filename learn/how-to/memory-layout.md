...
Zig's `@sizeOf`, `@alignOf` and `@offsetOf` report the real size, alignment
and field offsets of a struct. A plain struct may reorder its fields, so use
`extern struct` to match C and `packed struct` for exact bits.
...
places `b` at 0, `c` at 8, and `a` at 12, for a total of 16 bytes. Putting
the largest field first cuts the padding from eleven bytes to three.

These specific numbers belong to this compiler and this target. Do not depend
on the field order of an `auto` struct. If a byte offset matters, read it with `@offsetOf`, or use a layout that
guarantees order.
...
and 16 bytes. Use this when a buffer must be cache-line aligned or when
