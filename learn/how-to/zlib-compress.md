...
Zig compresses a buffer with the system zlib by linking `z`, translating the
`zlib.h` header, and calling `compressBound`, `compress2` and `uncompress` as
ordinary C functions.

## The problem

You need to shrink a buffer before writing or sending it, and zlib is the
library already on every machine. This recipe compresses a buffer and decompresses
...
pub fn main(init: std.process.Init) !void {
    ...
    // Repetitive data so the size reduction is large. (`**` string repeat
    // was removed from the language, so build it in a loop.)
...
zlib shows that a header and a library can have different names. The library is `z` (you link it with `-lz`), but the header is `zlib.h`. This
