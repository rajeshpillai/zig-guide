...
That gives a more useful way to see the problem. You do not need a rule that says `*` binds
...
This is **recursive descent**. Most production compilers use it, because the
...
at the *loosest* level, then expects a `)`. Nothing more is needed. Grouping
works for any expression because it restarts the chain.
...
and the answer would be 2 rather than -4. Right-associative operators, like
...
can be copied or thrown away in one step. Production compilers do this too, for the
...
Rust requires the braces and warns on the parentheses for exactly this reason.
...
There are two differences. Using indices rather than pointers is a choice
...
by giving every node the same fields, which is fine at thirteen node kinds
