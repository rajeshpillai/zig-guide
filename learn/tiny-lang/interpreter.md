...
Keeping them apart means a statement can never stand in for a value, because
`exec` has nothing to return. The grammar is what stops `print` appearing
inside an expression, so nothing has to check for that at run time.
