...
`lookup`: current frame first, globals second, and nothing else. Those two
lines are the complete scoping rule of this language.
...
one place: immediately after evaluating the body. Because nothing else catches
it, it works as a jump to a known point.

Interpreters use this technique on purpose. Interpreters written in languages
with exceptions do the same thing with a `ReturnException`. What
...
let a parameter see the new, empty frame instead of the caller's. That bug does
not crash. It gives wrong answers that look plausible.

**The stack depth is 64 and the limit is enforced.** `loop(n)` calling itself
forever stops with `StackOverflow`. It does not exhaust the host's stack and
crash the whole program. That is the advantage of building your own call
...
C creates the frame for you: locals live on the machine stack, and `return`
...
Compare two things with C. C's stack depth is whatever the OS gave the
...
comparison and gives you an error you can report with a line number.

And C's `return` unwinds nothing, because there is nothing to
