...
for the bytes to come from and somewhere for the answer to go. There is not
much new code here, because the parser is already built.
...
real port and checking what the client received. The missing Run button does
...
contain `\r\n\r\n`. This is the framing rule from the last chapter, and here
it matters in practice. The bytes are now trickling in from a socket, and
there is no way to know how many are coming.

**`Content-Length` on the response has a job.** Without it the client
...
on. Chunked encoding is the third way, and it exists because neither of the other
two works for a response whose length is not known when the headers are
sent.

**The routing is one `eql`.** `/` gets HTML, everything else gets a 404. At
this level, routing is only a comparison against the target.
...
**One connection per `serve` call, then it returns.** A production server wraps this
...
take. What happens *per connection* is unchanged, so it makes sense to study it
on its own.
...
with `GET /delete?id=5` links that a crawler visited.

Checking the method is one comparison. It matters because of what other
software assumes about `GET`.
...
This version avoids two C-specific hazards. The read into a fixed buffer has to
...
everything, as the [file descriptors](https://www.ziglang.in/learn/os/file-descriptors/) chapter
explains in detail.

## Where this section goes next

What is here is the protocol and the socket. What a production server adds, in
