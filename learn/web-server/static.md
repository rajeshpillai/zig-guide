...
that file under the document root, read it, send it. It is also a common way
to give an attacker your `/etc/passwd`, and the cause is a one-line mistake in
ordering.
...
`/%2e%2e/%2e%2e/etc/passwd`. The third is the one that gets through
servers in production, and it is refused here for one reason: `percentDecode` runs before
anything looks for `..`.

**Reject rather than normalise.** Collapsing `a/../b` into `b` is possible, and
then you must maintain a path normaliser, which is a second thing to get exactly right
...
different file gets opened. Zig's strings carry a length so this has no effect
...
The guard refuses `..` but accepts `/etc/passwd` with no dots at all.
...
request and the disk, but a directory handle is not a second guard. `openat`
resolves `..` like any other path, and it ignores the directory entirely when
the path is absolute. `std.Io.Dir` does offer `resolve_beneath` on its open
options, documented to turn an escaping path into `error.AccessDenied`, and
also documented to be ignored where the operating system does not support it.
Linux is one of those: the flag it is built on is a BSD `O_` flag, while Linux
spells the same idea as an `openat2` resolve flag, so the option silently does
nothing there. Passing it and opening `../outside.txt` still opens the file.
...
whoever maintains it toward a more complex check, and more complex checks are
how the encoded variant gets through.
...
a nul instead of ignoring it. In Zig the slice keeps its length, but the
moment the path crosses into C the length stops travelling with it.

`openat` with a directory handle is the usual shape, and in Zig it is
`dir.openFile`, which resolves relative to a `Dir`. It is not a sandbox. `..`
still climbs out of the directory and an absolute path ignores it, so the check
above still has to run whatever the platform does with `resolve_beneath`.
